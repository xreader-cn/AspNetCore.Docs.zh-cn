...
1. [应用程序生命周期](https://msdn.microsoft.com/library/ms227673.aspx)，是 ASP.NET 激发的一系列事件：[BeginRequest](/dotnet/api/system.web.httpapplication.beginrequest)、 [AuthenticateRequest](/dotnet/api/system.web.httpapplication.authenticaterequest)等。每个模块都可以为一个或多个事件创建处理程序。
...
中所示[中间件](xref:fundamentals/middleware/index)页上，ASP.NET Core 中间件是公开的类`Invoke`方法拍摄`HttpContext`并返回`Task`。 新的中间件将如下所示：
...
某些模块和处理程序具有*存储在 web.config 中的配置*选项。但是，在 ASP.NET Core 中新的配置模型使用代替了*Web.config*。
...
4. 将选项注入中间件构造函数。 这类似于将选项注入控制器。
   ...
   将[中间件](#http-modules-usemiddleware)添加到中的 UseMiddleware 扩展方法`IApplicationBuilder`会处理依赖关系注入。
...
`HttpContext` 已显著更改 ASP.NET Core 中。 本部分演示如何将 system.servicemodel 最[常用的属性转换为新](/dotnet/api/system.web.httpcontext) `Microsoft.AspNetCore.Http.HttpContext`的。
