...
对于面向.NET Framework 的 ASP.NET Core 应用不支持进程内承载模型。
...
如果该应用程序调用<xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*>中`CreateDefaultBuilder`方法`Program`类中，调用`ConfigureKestrel`若要配置 Kestrel 服务器，而不是`UseKestrel`以避免与冲突[IIS 进程托管模型](xref:fundamentals/servers/index#in-process-hosting-model):
...
如果应用程序不会调用`CreateDefaultBuilder`，调用<xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*>**之前**调用`ConfigureKestrel`:
