...
* 删除对引用 **&lt;DotNetCliToolReference&gt;** 以下包元素。 默认情况下，这些工具在 .NET Core CLI 中捆绑在一起，无需单独安装。
  * DotNet （`dotnet watch`）
  * Microsoft.EntityFrameworkCore.Tools.DotNet (`dotnet ef`)
  * SqlConfig （`dotnet sql-cache`）（& e）
  * SecretManager （`dotnet user-secrets`）
* 可选： 你可以删除 **&lt;DotNetCliToolReference&gt;** 元素`Microsoft.VisualStudio.Web.CodeGeneration.Tools`。 可以通过运行 `dotnet tool install -g dotnet-aspnet-codegenerator`将此工具替换为全局安装的版本。
...
* 删除现有的标识2.0 代码，并将[基架标识](xref:security/authentication/scaffold-identity)导入到项目中。 你的项目将使用[ASP.NET Core 标识](xref:security/authentication/identity) [Razor 类库](xref:razor-pages/ui-class)。 你可以为你修改的任何标识 UI 代码生成代码和 UI。 将代码更改应用于新的基架 UI 代码。
...
本部分概述的步骤，以替换与的 ASP.NET Core 2.0 模板生成标识代码[ASP.NET Core 标识](xref:security/authentication/identity) [Razor 类库](xref:razor-pages/ui-class)。 以下步骤适用于 Razor Pages 项目，但 MVC 项目的方法类似。
