...
* 删除的包引用的"版本"属性`Microsoft.AspNetCore.App`。 项目使用`<Project Sdk="Microsoft.NET.Sdk.Web">`不需要设置的版本。 版本为权限隐含的目标框架，并选择最能满足 ASP.NET Core 2.1 的工作的方式。 有关详细信息，请参阅[面向共享的框架的项目的规则](#rules-for-projects-targeting-the-shared-framework)部分。
...
一个*共享的框架*是一组程序集 (*.dll*文件) 不是在应用程序的文件夹中。 若要运行该应用程序在计算机上必须安装共享的框架。 有关详细信息，请参阅[共享的框架](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/)。
...
<xref:Microsoft.AspNetCore.Mvc.FileResult> 不能再处理[Accept-ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges)默认情况下的标头。 若要启用`Accept-Ranges`标头，设置<xref:Microsoft.AspNetCore.Mvc.FileResult.EnableRangeProcessing>到`true`。

### <a name="controllerbasefile-and-physicalfile-range-header"></a>ControllerBase.File 和 PhysicalFile 范围标头

以下<xref:Microsoft.AspNetCore.Mvc.ControllerBase>方法不能再处理[Accept-ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges)默认情况下的标头：

* 重载的[ControllerBase.File](/dotnet/api/microsoft.aspnetcore.mvc.controllerbase.file#Microsoft_AspNetCore_Mvc_ControllerBase_File_System_String_System_String_System_String_System_Boolean_)
* [ControllerBase.PhysicalFile](xref:Microsoft.AspNetCore.Mvc.ControllerBase.PhysicalFile*)

若要启用`Accept-Ranges`标头，设置`EnableRangeProcessing`参数`true`。
