...
* 设置[目标框架名字对象 (TFM)](/dotnet/standard/frameworks)到`netcoreapp3.0`:
...
* 更新`Version`特性上剩余`<PackageReference>`元素`Microsoft.AspNetCore.*`程序包添加到当前的预览版中 (例如， `3.0.0-preview5-19227-01`)。

  如果没有版本 3.0 的包，包可能会弃用在 3.0 中。 许多这些包属于`Microsoft.AspNetCore.App`，并且不应单独引用。 初步不再生成在 3.0 中的包的列表，请参阅[停止在 3.0 中的共享的框架程序集生成包 (aspnet/AspNetCore #3756)](https://github.com/aspnet/AspNetCore/issues/3756)。 *共享的框架*是程序集的集合 ( *.dll*文件)，是在计算机上安装并引用`Microsoft.AspNetCore.App`。 有关详细信息，请参阅[共享框架](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/)。

* 值得注意的几个组件的程序集已从`Microsoft.AspNetCore.App`在 3.0 中。 添加`<PackageReference>`如果你使用的 Api 中列出的包中的元素[正在从 Microsoft.AspNetCore.App 3.0 中删除的程序集 (aspnet/AspNetCore #3755)](https://github.com/aspnet/AspNetCore/issues/3755)。
  ...
  * `System.Data.SqlClient`
  ...
  在进行发布的程序集列表`Microsoft.AspNetCore.App`尚未完成，并且在 3.0 RTM 之前会更改。
  ...
  `ReadAsAsync`中包含在前面的代码中调用的方法`Microsoft.AspNet.WebApi.Client`。 安装[Microsoft.AspNet.WebApi.Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) NuGet 程序包来解决 3.0 中的编译问题。
...
* 更新`Startup.ConfigureServices`调用`AddNewtonsoftJson`。
  ...
  `AddNewtonsoftJson` 适用于新的 MVC 服务注册方法：
  ...
  * `AddControllers`
  ...
  对的调用中，可以设置 Json.NET 设置`AddNewtonsoftJson`:
...
在早期版本的 ASP.NET Core，身份验证支持通过提供`[Authorize]`属性。 授权中间件不可用。 在 ASP.NET Core 3.0 中，授权中间件是必需的。 建议您将 ASP.NET Core 授权中间件 (`UseAuthorization`) 后立即`UseAuthentication`。 此外可以使用默认策略，可以重写配置授权中间件。
...
`System.Text.Json` 现在，中心协议的客户端和服务器使用的默认值。
...
如果使用的功能`Newtonsoft.Json`中不支持`System.Text.Json`，可以切换回`Newtonsoft.Json`:

1. 安装[Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson) NuGet 包。
1. 在客户端上链接`AddNewtonsoftJsonProtocol`方法调用`HubConnectionBuilder`实例：
...
1. 在服务器上链接`AddNewtonsoftJsonProtocol`方法调用`AddSignalR`方法调用中`Startup.ConfigureServices`:
...
在 3.0 中，运行时编译是选择的方案。 若要启用运行时编译，请参阅 https://docs.microsoft.com/aspnet/core/mvc/views/view-compilation?view=aspnetcore-3.0#runtime-compilation 。
