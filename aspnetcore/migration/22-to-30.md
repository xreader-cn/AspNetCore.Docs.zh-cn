...
* 在 `<PropertyGroup>`：
  ...
  * 删除 @no__t 0 元素。 有关详细信息，请参阅本文档中的[进程内承载模型](#in-process-hosting-model)。
...
* MVC `Newtonsoft.Json` @no__t 支持对使用带有 `Newtonsoft.Json` 的 MVC-2 现在是[NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson)的一部分。
