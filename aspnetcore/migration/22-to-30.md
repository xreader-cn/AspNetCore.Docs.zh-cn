...
从 `WebHostBuilder` 到 `HostBuilder` 的最重大更改是在[依赖关系注入（DI）](xref:fundamentals/dependency-injection)中进行的。 使用 `HostBuilder` 时，只能将 <xref:Microsoft.Extensions.Configuration.IConfiguration> 和 <xref:Microsoft.AspNetCore.Hosting.IHostingEnvironment> 插入 `Startup` 的构造函数中。 @No__t_0 DI 约束：
