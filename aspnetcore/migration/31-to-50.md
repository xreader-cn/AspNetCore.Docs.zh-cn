...
1. 在中 `Shared/MainLayout.razor` ，将所有内容括在元素中， `<div>` 其为 `id` `page` ：
...
1. 最新的基 `wwwroot/css/app.css` 文件包含以下样式。 删除附加样式，使其保留以下样式和任何已添加到应用的样式。

    `wwwroot/css/app.css` (基本样式，不包括开发人员) 添加的样式：
