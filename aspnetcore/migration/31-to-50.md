...
## <a name="changes-to-model-binding-in-aspnet-core-mvc-and-no-locrazor-pages"></a>ASP.NET Core MVC 和页面中对模型绑定的更改 Razor

### <a name="datetime-values-are-model-bound-as-utc-times"></a>DateTime 值是作为 UTC 时间的模型绑定

在 ASP.NET Core 3.1 及更早版本中， `DateTime` 值作为本地时间绑定，时区由服务器确定。 `DateTime` 绑定自输入格式 (JSON) 和值的值 `DateTimeOffset` 被绑定为 UTC 时区。 在 ASP.NET Core 5.0 及更高版本中，模型绑定 `DateTime` 将值与 UTC 时区一致地绑定在一起。

若要保留以前的行为，请在 `DateTimeModelBinderProvider` 应用程序启动过程中删除：
...
在为模型绑定记录类型添加支持的过程中， `ComplexTypeModelBinderProvider` 会将标注为已过时，不再会被默认注册。 之前依赖于集合中是否存在的应用程序 `ComplexTypeModelBinderProvider` `ModelBinderProviders` 需要更新才能引用新的联编程序提供程序：
