...
---
# <a name="migrate-from-aspnet-membership-authentication-to-aspnet-core-20-identity"></a>从 ASP.NET 成员身份验证迁移到 ASP.NET Core 2.0 标识

作者：[Isaac Levin](https://isaaclevin.com)

本文演示了如何使用成员身份验证将 ASP.NET 应用的数据库架构迁移到 ASP.NET Core 2.0 标识。

> [!NOTE]
> 本文档提供将基于 ASP.NET 成员身份的应用的数据库架构迁移到用于 ASP.NET Core 标识的数据库架构所需的步骤。 有关从基于 ASP.NET 成员身份的身份验证迁移到 ASP.NET Identity 的详细信息，请参阅[将现有应用从 SQL 成员身份迁移到 ASP.NET Identity](/aspnet/identity/overview/migrations/migrating-an-existing-website-from-sql-membership-to-aspnet-identity)。 有关 ASP.NET Core 标识的详细信息，请参阅[ASP.NET Core 上的标识简介](xref:security/authentication/identity)。
...
若要将现有应用迁移到 ASP.NET Core 2.0 标识，需要将这些表中的数据迁移到新的标识架构所使用的表中。

## <a name="aspnet-core-identity-20-schema"></a>ASP.NET Core 标识2.0 架构
...
查看 ASP.NET Core 2.0 标识的架构的最快方法是创建新的 ASP.NET Core 2.0 应用。 在 Visual Studio 2017 中执行以下步骤：
...
1. 为标识模板选择**单个用户帐户**。 最后，单击 **"确定**"，然后单击 **"确定"**。 Visual Studio 使用 ASP.NET Core 标识模板来创建项目。
...
1. 选择 "**查看** > **SQL Server 对象资源管理器**"。 展开对应于`ConnectionStrings:DefaultConnection` *appsettings*的属性中指定的数据库名称的节点。
...
成员身份和 ASP.NET Core 标识的表结构和字段之间存在细微的差异。 此模式已因 ASP.NET 和 ASP.NET Core 应用的身份验证/授权而发生了重大更改。 仍用于标识的关键对象是*用户*和*角色*。 下面是*用户*、*角色*和*UserRoles*的映射表。

### <a name="users"></a>用户

|*标识<br>（dbo。AspNetUsers*        ||*成员<br>身份（aspnet_Users/dbo. aspnet_Membership）*||
|----------------------------------------|-----------------------------------------------------------|
|**字段名称**                 |**类型**|**字段名称**                                    |**类型**|
...
> [!NOTE]
> 并非所有字段映射都类似于从成员身份到 ASP.NET Core 标识的一对一关系。 上表采用默认的成员资格用户架构，并将其映射到 ASP.NET Core 的标识架构。 需要手动映射用于成员身份的任何其他自定义字段。 在此映射中，无密码映射，因为密码条件和密码 salts 不会在两者之间迁移。 **建议将密码保留为 null 并要求用户重置其密码。** 在 ASP.NET Core 标识中`LockoutEnd` ，如果用户被锁定，则应设置为将来的某个日期。这会显示在迁移脚本中。

### <a name="roles"></a>角色

|*标识<br>（dbo。AspNetRoles)*        ||*成员<br>身份（aspnet_Roles）*||
|----------------------------------------|-----------------------------------|
|**字段名称**                 |**类型**|**字段名称**   |**类型**         |
...
|*标识<br>（dbo。AspNetUserRoles*||*成员<br>身份（aspnet_UsersInRoles）*||
|------------------------------------|------------------------------------------|
|**字段名称**           |**类型**  |**字段名称**|**类型**                   |
...
完成上述脚本后，将向成员资格Identity用户填充先前创建的 ASP.NET Core 应用。 用户需要在登录之前更改密码。

> [!NOTE]
> 如果成员资格系统中的用户的用户名与其电子邮件地址不匹配，则需要对之前创建的应用进行更改，以满足此要求。 默认模板需要`UserName`和`Email`相同。 对于不同的情况，需要将登录过程修改为使用`UserName`而不是。 `Email`

在登录`PageModel`页的 " *Pages\Account\Login.cshtml.cs*" 中，从 " `[EmailAddress]` *电子邮件*" 属性中删除该属性。 将其重命名为*UserName*。 这需要在*视图*和`EmailAddress` *PageModel*中提到的任何位置进行更改。 结果应类似如下所示：

 ![固定登录](identity/_static/fixed-login.png)

## <a name="next-steps"></a>后续步骤

在本教程中，已了解如何将用户从 SQL 成员身份移植到IdentityASP.NET Core 2.0。 有关 ASP.NET Core Identity的详细信息，请参阅[介绍Identity ](xref:security/authentication/identity)。
