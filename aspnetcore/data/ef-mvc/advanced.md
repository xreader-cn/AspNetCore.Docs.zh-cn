...
假设 Contoso 大学管理员想要在数据库中执行全局更改，例如如更改的每个课程的学分。 如果该大学有大量的课程，检索所有实体并单独更改会降低效率。 在本节中，你将实现一个页面，使用户能够指定一个参数，通过这个参数可以更改所有课程的学分，在这里你会通过执行 SQL UPDATE 语句来进行更改。 页面外观类似于下图：
...
单击**Back To List**可以看到课程列表，其中学分已经替换成修改后的数字。

请注意生产代码将确保更新最终得到有效的数据。 此处所示的简化代码会使得相乘后学分大于 5。 (`Credits`属性具有`[Range(0, 5)]`特性。)更新查询将起作用，但无效的数据会导致意外的结果，例如在系统的其他部分中加入学分为 5 或更少可能会导致意外的结果。
