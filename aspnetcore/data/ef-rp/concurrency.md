...
### <a name="pessimistic-concurrency-locking"></a><span data-ttu-id="a5e8f-112">悲观并发（锁定）</span><span class="sxs-lookup"><span data-stu-id="a5e8f-112">Pessimistic concurrency (locking)</span></span>
...
* <span data-ttu-id="a5e8f-129">可以跟踪用户已修改的属性，并仅更新数据库中相应的列。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-129">You can keep track of which property a user has modified and update only the corresponding columns in the database.</span></span>
  ...
  <span data-ttu-id="a5e8f-130">在这种情况下，数据不会丢失。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-130">In the scenario, no data would be lost.</span></span> <span data-ttu-id="a5e8f-131">两个用户更新了不同的属性。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-131">Different properties were updated by the two users.</span></span> <span data-ttu-id="a5e8f-132">下次有人浏览英语系时，将看到 Jane 和 John 两个人的更改。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-132">The next time someone browses the English department, they will see both Jane's and John's changes.</span></span> <span data-ttu-id="a5e8f-133">这种更新方法可以减少导致数据丢失的冲突数。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-133">This method of updating can reduce the number of conflicts that could result in data loss.</span></span> <span data-ttu-id="a5e8f-134">这种方法具有一些缺点：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-134">This approach has some disadvantages:</span></span>
...
## <a name="conflict-detection-in-ef-core"></a><span data-ttu-id="a5e8f-152">EF Core 中的冲突检测</span><span class="sxs-lookup"><span data-stu-id="a5e8f-152">Conflict detection in EF Core</span></span>
...
## <a name="add-a-tracking-property"></a><span data-ttu-id="a5e8f-169">添加跟踪属性</span><span class="sxs-lookup"><span data-stu-id="a5e8f-169">Add a tracking property</span></span>
...
<span data-ttu-id="a5e8f-183">以下代码显示更新 Department 名称时由 EF Core 生成的部分 T-SQL：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-183">The following code shows a portion of the T-SQL generated by EF Core when the Department name is updated:</span></span>
...
* <span data-ttu-id="a5e8f-211">在 PMC 中运行以下命令：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-211">Run the following command in the PMC:</span></span>
...
# <a name="visual-studio-code"></a>[<span data-ttu-id="a5e8f-224">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="a5e8f-224">Visual Studio Code</span></span>](#tab/visual-studio-code)
...
<span data-ttu-id="a5e8f-238">以下代码显示更新后的页面：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-238">The following code shows the updated page:</span></span>
...
<span data-ttu-id="a5e8f-251">发生并发错误时，以下突出显示的代码会获取客户端值（发布到此方法的值）和数据库值。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-251">When a concurrency error happens, the following highlighted code gets the client values (the values posted to this method) and the database values.</span></span>
...
### <a name="test-concurrency-conflicts-with-the-edit-page"></a><span data-ttu-id="a5e8f-265">使用编辑页测试并发冲突</span><span class="sxs-lookup"><span data-stu-id="a5e8f-265">Test concurrency conflicts with the Edit page</span></span>
...
## <a name="update-the-delete-page"></a><span data-ttu-id="a5e8f-286">更新“删除”页</span><span class="sxs-lookup"><span data-stu-id="a5e8f-286">Update the Delete page</span></span>
...
### <a name="test-concurrency-conflicts"></a><span data-ttu-id="a5e8f-304">测试并发冲突</span><span class="sxs-lookup"><span data-stu-id="a5e8f-304">Test concurrency conflicts</span></span>
...
<span data-ttu-id="a5e8f-321">这是本系列的最后一个教程。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-321">This is the last tutorial in the series.</span></span> <span data-ttu-id="a5e8f-322">[本系列教程的 MVC 版本](xref:data/ef-mvc/index)中介绍了其他主题。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-322">Additional topics are covered in the [MVC version of this tutorial series](xref:data/ef-mvc/index).</span></span>
...
<span data-ttu-id="a5e8f-374">数据库和数据模型必须配置为支持引发 `DbUpdateConcurrencyException`。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-374">The DB and data model must be configured to support throwing `DbUpdateConcurrencyException`.</span></span>
...
<span data-ttu-id="a5e8f-392">在 EF Core 中，如果未通过 `Update` 或 `Delete` 命令更新行，则引发并发异常。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-392">In EF Core, when no rows have been updated by an `Update` or `Delete` command, a concurrency exception is thrown.</span></span>
...
<span data-ttu-id="a5e8f-404">在 Visual Studio 的输出窗口中可看见 EF Core 生成的 T-SQL。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-404">You can see the T-SQL EF Core generates in the output window of Visual Studio.</span></span>
...
# <a name="visual-studio-code"></a>[<span data-ttu-id="a5e8f-417">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="a5e8f-417">Visual Studio Code</span></span>](#tab/visual-studio-code)
...
### <a name="update-the-edit-page-model"></a><span data-ttu-id="a5e8f-432">更新编辑页模型</span><span class="sxs-lookup"><span data-stu-id="a5e8f-432">Update the Edit page model</span></span>
...
<span data-ttu-id="a5e8f-443">`ModelState` 具有旧的 `RowVersion` 值，因此需使用 `ModelState.Remove` 语句。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-443">The `ModelState.Remove` statement is required because `ModelState` has the old `RowVersion` value.</span></span> <span data-ttu-id="a5e8f-444">在 Razor 页面中，当两者都存在时，字段的 `ModelState` 值优于模型属性值。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-444">In the Razor Page, the `ModelState` value for a field takes precedence over the model property values when both are present.</span></span>

## <a name="update-the-edit-page"></a><span data-ttu-id="a5e8f-445">更新“编辑”页</span><span class="sxs-lookup"><span data-stu-id="a5e8f-445">Update the Edit page</span></span>

<span data-ttu-id="a5e8f-446">使用以下标记更新 Pages/Departments/Edit.cshtml  ：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-446">Update *Pages/Departments/Edit.cshtml* with the following markup:</span></span>

[!code-html[](intro/samples/cu/Pages/Departments/Edit.cshtml?highlight=1,14,16-17,37-39)]

<span data-ttu-id="a5e8f-447">前面的标记：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-447">The preceding markup:</span></span>

* <span data-ttu-id="a5e8f-448">将 `page` 指令从 `@page` 更新为 `@page "{id:int}"`。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-448">Updates the `page` directive from `@page` to `@page "{id:int}"`.</span></span>
* <span data-ttu-id="a5e8f-449">添加隐藏的行版本。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-449">Adds a hidden row version.</span></span> <span data-ttu-id="a5e8f-450">必须添加 `RowVersion`，以便回发绑定值。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-450">`RowVersion` must be added so post back binds the value.</span></span>
* <span data-ttu-id="a5e8f-451">显示 `RowVersion` 的最后一个字节以进行调试。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-451">Displays the last byte of `RowVersion` for debugging purposes.</span></span>
* <span data-ttu-id="a5e8f-452">将 `ViewData` 替换为强类型 `InstructorNameSL`。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-452">Replaces `ViewData` with the strongly-typed `InstructorNameSL`.</span></span>

## <a name="test-concurrency-conflicts-with-the-edit-page"></a><span data-ttu-id="a5e8f-453">使用编辑页测试并发冲突</span><span class="sxs-lookup"><span data-stu-id="a5e8f-453">Test concurrency conflicts with the Edit page</span></span>

<span data-ttu-id="a5e8f-454">在英语系打开编辑的两个浏览器实例：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-454">Open two browsers instances of Edit on the English department:</span></span>

* <span data-ttu-id="a5e8f-455">运行应用，然后选择“院系”。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-455">Run the app and select Departments.</span></span>
* <span data-ttu-id="a5e8f-456">右键单击英语系的“编辑”超链接，然后选择“在新选项卡中打开”   。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-456">Right-click the **Edit** hyperlink for the English department and select **Open in new tab**.</span></span>
* <span data-ttu-id="a5e8f-457">在第一个选项卡中，单击英语系的“编辑”超链接  。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-457">In the first tab, click the **Edit** hyperlink for the English department.</span></span>

<span data-ttu-id="a5e8f-458">两个浏览器选项卡显示相同信息。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-458">The two browser tabs display the same information.</span></span>

<span data-ttu-id="a5e8f-459">在第一个浏览器选项卡中更改名称，然后单击“保存”  。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-459">Change the name in the first browser tab and click **Save**.</span></span>

![更改后的“院系编辑”页 1](concurrency/_static/edit-after-change-1.png)

<span data-ttu-id="a5e8f-461">浏览器显示更改值并更新 rowVersion 标记后的索引页。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-461">The browser shows the Index page with the changed value and updated rowVersion indicator.</span></span> <span data-ttu-id="a5e8f-462">请注意更新后的 rowVersion 标记，它在其他选项卡的第二回发中显示。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-462">Note the updated rowVersion indicator, it's displayed on the second postback in the other tab.</span></span>

<span data-ttu-id="a5e8f-463">在第二个浏览器选项卡中更改不同字段。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-463">Change a different field in the second browser tab.</span></span>

![更改后的“院系编辑”页 2](concurrency/_static/edit-after-change-2.png)

<span data-ttu-id="a5e8f-465">单击“保存”  。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-465">Click **Save**.</span></span> <span data-ttu-id="a5e8f-466">可看见所有不匹配数据库值的字段的错误消息：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-466">You see error messages for all fields that don't match the DB values:</span></span>

![“院系编辑”页错误消息](concurrency/_static/edit-error.png)

<span data-ttu-id="a5e8f-468">此浏览器窗口将不会更改名称字段。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-468">This browser window didn't intend to change the Name field.</span></span> <span data-ttu-id="a5e8f-469">将当前值（语言）复制并粘贴到名称字段。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-469">Copy and paste the current value (Languages) into the Name field.</span></span> <span data-ttu-id="a5e8f-470">退出选项卡。客户端验证将删除错误消息。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-470">Tab out. Client-side validation removes the error message.</span></span>

![“院系编辑”页错误消息](concurrency/_static/cv.png)

<span data-ttu-id="a5e8f-472">再次单击“保存”  。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-472">Click **Save** again.</span></span> <span data-ttu-id="a5e8f-473">保存在第二个浏览器选项卡中输入的值。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-473">The value you entered in the second browser tab is saved.</span></span> <span data-ttu-id="a5e8f-474">在索引页中可以看到保存的值。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-474">You see the saved values in the Index page.</span></span>

## <a name="update-the-delete-page"></a><span data-ttu-id="a5e8f-475">更新“删除”页</span><span class="sxs-lookup"><span data-stu-id="a5e8f-475">Update the Delete page</span></span>

<span data-ttu-id="a5e8f-476">使用以下代码更新“删除”页模型：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-476">Update the Delete page model with the following code:</span></span>

[!code-csharp[](intro/samples/cu/Pages/Departments/Delete.cshtml.cs)]

<span data-ttu-id="a5e8f-477">删除页检测提取实体并更改时的并发冲突。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-477">The Delete page detects concurrency conflicts when the entity has changed after it was fetched.</span></span> <span data-ttu-id="a5e8f-478">提取实体后，`Department.RowVersion` 为行版本。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-478">`Department.RowVersion` is the row version when the entity was fetched.</span></span> <span data-ttu-id="a5e8f-479">EF Core 创建 SQL DELETE 命令时，它包括具有 `RowVersion` 的 WHERE 子句。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-479">When EF Core creates the SQL DELETE command, it includes a WHERE clause with `RowVersion`.</span></span> <span data-ttu-id="a5e8f-480">如果 SQL DELETE 命令导致零行受影响：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-480">If the SQL DELETE command results in zero rows affected:</span></span>

* <span data-ttu-id="a5e8f-481">SQL DELETE 命令中的 `RowVersion` 与数据库中的 `RowVersion` 不匹配。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-481">The `RowVersion` in the SQL DELETE command doesn't match `RowVersion` in the DB.</span></span>
* <span data-ttu-id="a5e8f-482">引发 DbUpdateConcurrencyException 异常。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-482">A DbUpdateConcurrencyException exception is thrown.</span></span>
* <span data-ttu-id="a5e8f-483">使用 `concurrencyError` 调用 `OnGetAsync`。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-483">`OnGetAsync` is called with the `concurrencyError`.</span></span>

### <a name="update-the-delete-page"></a><span data-ttu-id="a5e8f-484">更新“删除”页</span><span class="sxs-lookup"><span data-stu-id="a5e8f-484">Update the Delete page</span></span>

<span data-ttu-id="a5e8f-485">使用以下代码更新 Pages/Departments/Delete.cshtml  ：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-485">Update *Pages/Departments/Delete.cshtml* with the following code:</span></span>

[!code-html[](intro/samples/cu/Pages/Departments/Delete.cshtml?highlight=1,10,39,51)]

<span data-ttu-id="a5e8f-486">上面的代码执行以下更改：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-486">The preceding code makes the following changes:</span></span>

* <span data-ttu-id="a5e8f-487">将 `page` 指令从 `@page` 更新为 `@page "{id:int}"`。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-487">Updates the `page` directive from `@page` to `@page "{id:int}"`.</span></span>
* <span data-ttu-id="a5e8f-488">添加错误消息。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-488">Adds an error message.</span></span>
* <span data-ttu-id="a5e8f-489">将“管理员”字段中的 FirstMidName 替换为 FullName  。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-489">Replaces FirstMidName with FullName in the **Administrator** field.</span></span>
* <span data-ttu-id="a5e8f-490">更改 `RowVersion` 以显示最后一个字节。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-490">Changes `RowVersion` to display the last byte.</span></span>
* <span data-ttu-id="a5e8f-491">添加隐藏的行版本。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-491">Adds a hidden row version.</span></span> <span data-ttu-id="a5e8f-492">必须添加 `RowVersion`，以便回发绑定值。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-492">`RowVersion` must be added so post back binds the value.</span></span>

### <a name="test-concurrency-conflicts-with-the-delete-page"></a><span data-ttu-id="a5e8f-493">使用删除页测试并发冲突</span><span class="sxs-lookup"><span data-stu-id="a5e8f-493">Test concurrency conflicts with the Delete page</span></span>

<span data-ttu-id="a5e8f-494">创建测试系。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-494">Create a test department.</span></span>

<span data-ttu-id="a5e8f-495">在测试系打开删除的两个浏览器实例：</span><span class="sxs-lookup"><span data-stu-id="a5e8f-495">Open two browsers instances of Delete on the test department:</span></span>

* <span data-ttu-id="a5e8f-496">运行应用，然后选择“院系”。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-496">Run the app and select Departments.</span></span>
* <span data-ttu-id="a5e8f-497">右键单击测试系的“删除”超链接，然后选择“在新选项卡中打开”   。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-497">Right-click the **Delete** hyperlink for the test department and select **Open in new tab**.</span></span>
* <span data-ttu-id="a5e8f-498">单击测试系的“编辑”超链接  。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-498">Click the **Edit** hyperlink for the test department.</span></span>

<span data-ttu-id="a5e8f-499">两个浏览器选项卡显示相同信息。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-499">The two browser tabs display the same information.</span></span>

<span data-ttu-id="a5e8f-500">在第一个浏览器选项卡中更改预算，然后单击“保存”  。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-500">Change the budget in the first browser tab and click **Save**.</span></span>

<span data-ttu-id="a5e8f-501">浏览器显示更改值并更新 rowVersion 标记后的索引页。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-501">The browser shows the Index page with the changed value and updated rowVersion indicator.</span></span> <span data-ttu-id="a5e8f-502">请注意更新后的 rowVersion 标记，它在其他选项卡的第二回发中显示。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-502">Note the updated rowVersion indicator, it's displayed on the second postback in the other tab.</span></span>

<span data-ttu-id="a5e8f-503">从第二个选项卡中删除测试部门。并发错误显示来自数据库的当前值。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-503">Delete the test department from the second tab. A concurrency error is display with the current values from the DB.</span></span> <span data-ttu-id="a5e8f-504">单击“删除”将删除实体，除非 `RowVersion` 已更新，院系已删除  。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-504">Clicking **Delete** deletes the entity, unless `RowVersion` has been updated.department has been deleted.</span></span>

<span data-ttu-id="a5e8f-505">请参阅[继承](xref:data/ef-mvc/inheritance)了解如何继承数据模型。</span><span class="sxs-lookup"><span data-stu-id="a5e8f-505">See [Inheritance](xref:data/ef-mvc/inheritance) on how to inherit a data model.</span></span>

### <a name="additional-resources"></a><span data-ttu-id="a5e8f-506">其他资源</span><span class="sxs-lookup"><span data-stu-id="a5e8f-506">Additional resources</span></span>

* [<span data-ttu-id="a5e8f-507">EF Core 中的并发令牌</span><span class="sxs-lookup"><span data-stu-id="a5e8f-507">Concurrency Tokens in EF Core</span></span>](/ef/core/modeling/concurrency)
* [<span data-ttu-id="a5e8f-508">EF Core 中的并发处理</span><span class="sxs-lookup"><span data-stu-id="a5e8f-508">Handle concurrency in EF Core</span></span>](/ef/core/saving/concurrency)
* [<span data-ttu-id="a5e8f-509">本教程的 YouTube 版本（处理并发冲突）</span><span class="sxs-lookup"><span data-stu-id="a5e8f-509">YouTube version of this tutorial(Handling Concurrency Conflicts)</span></span>](https://youtu.be/EosxHTFgYps)
* [<span data-ttu-id="a5e8f-510">本教程的 YouTube 版本（第 2 部分）</span><span class="sxs-lookup"><span data-stu-id="a5e8f-510">YouTube version of this tutorial(Part 2)</span></span>](https://www.youtube.com/watch?v=kcxERLnaGO0)
* [<span data-ttu-id="a5e8f-511">本教程的 YouTube 版本（第 3 部分）</span><span class="sxs-lookup"><span data-stu-id="a5e8f-511">YouTube version of this tutorial(Part 3)</span></span>](https://www.youtube.com/watch?v=d4RbpfvELRs)

> [!div class="step-by-step"]
> [<span data-ttu-id="a5e8f-512">上一篇</span><span class="sxs-lookup"><span data-stu-id="a5e8f-512">Previous</span></span>](xref:data/ef-rp/update-related-data)
