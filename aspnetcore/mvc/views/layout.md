...
布局可以通过调用  *来选择引用一个或多个节*`RenderSection`。 节提供一种方法来组织某些页面元素应当放置的位置。 每次调用 `RenderSection` 时都可指定该部分是必需还是可选：
...
在上面的代码中，scripts / main.js 添加到了页面或视图的  *部分*`scripts`。 相同应用中的其他页面或视图可能不需要此脚本且不会定义脚本部分。
...
* 根级别 _ViewImports.cshtml 文件包含 *和*。`@model MyModel1``@addTagHelper *, MyTagHelper1`
* 子文件夹 _ViewImports.cshtml 文件包含 *和*。`@model MyModel2``@addTagHelper *, MyTagHelper2`
