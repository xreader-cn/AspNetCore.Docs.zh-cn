...
此类别更为熟知的说法是“内容协商”。 每当操作返回 [ObjectResult](xref:web-api/advanced/formatting#content-negotiation) 类型或除 [IActionResult](/dotnet/api/microsoft.aspnetcore.mvc.objectresult) 实现之外的任何实现时，会应用[内容协商](/dotnet/api/microsoft.aspnetcore.mvc.iactionresult)。 返回非 `IActionResult` 实现（例如 `object`）的操作也会返回已格式化的响应。
