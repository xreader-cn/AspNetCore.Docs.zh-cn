...
* 对于基于 ASP.NET Core MVC 或[Blazor](xref:blazor/index)项目，生成、打包和发布包含[Razor](xref:mvc/views/razor)文件的项目是必需的。
...
若要使用 Razor SDK 生成包含 Razor 视图或 Razor Pages 的类库，我们建议从 Razor 类库（RCL）项目模板开始。 用于构建 Blazor （*razor*）文件的 RCL 至少需要一个[AspNetCore](https://www.nuget.org/packages/Microsoft.AspNetCore.Components)包的引用。 用于生成 Razor 视图或页面（*cshtml*文件）的 RCL 最低要求目标 `netcoreapp3.0` 或更高版本，并在其项目文件中具有[AspNetCore 元包](xref:fundamentals/metapackage-app)的 `FrameworkReference`。
...
| `GenerateRazorTargetAssemblyInfo` | `true`时，将生成一个 *.cs*文件，其中包含 `RazorAssemblyAttribute` 指定的特性，并在编译输出中包含该文件。 |
| `EnableDefaultRazorTargetAssemblyInfoAttributes` | 为 `true` 时，将一组默认的程序集属性添加到 `RazorAssemblyAttribute`。 |
| `CopyRazorGenerateFilesToPublishDirectory` | `true`时，将 `RazorGenerate` 项（*cshtml*）文件复制到发布目录。 通常，如果发布的应用在生成时或发布时参与编译，则不需要 Razor 文件。 默认为 `false`。 |
