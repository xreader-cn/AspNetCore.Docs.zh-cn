...
* 本主题稍后会添加一个 `{aboutTemplate?}` 的路由模板。 为 `{aboutTemplate?}` 模板指定的 `Order` 为 `2`。 当在 `/About/RouteDataValue` 中请求“关于”页面时，由于设置了 `Order` 属性，“RouteDataValue”会加载到 `RouteData.Values["globalTemplate"]` (`Order = 1`) 而不是 `RouteData.Values["aboutTemplate"]` (`Order = 2`) 中。
...
@No__t_0 页路由模型约定将参数变压器应用到应用中自动生成的页面路由的文件夹和文件名段。 例如，位于 */Pages/SubscriptionManagement/ViewAll.cshtml*的 Razor Pages 文件会将其路由从 `/SubscriptionManagement/ViewAll` 重写到 `/subscription-management/view-all` 中。
...
@No__t_0 注册为 `Startup.ConfigureServices` 中的一个选项：
