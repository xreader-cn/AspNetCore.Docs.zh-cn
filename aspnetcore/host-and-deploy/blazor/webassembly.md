---
title:“托管和部署 ASP.NET Core Blazor WebAssembly”author: description:“了解如何使用 ASP.NET Core、内容分发网络 (CDN)、文件服务器和 GitHub 页来托管和部署 Blazor 应用。”
...
------ | | `type`       | 资源类型。 允许使用的类型：`assembly`、`pdb`、`dotnetjs`、`dotnetwasm`、`timezonedata` | | `name`       | 资源名称。 | | `defaultUri` | 资源的相对或绝对 URI。 | | `integrity`  | 表示响应中预期内容的完整性字符串。 |
