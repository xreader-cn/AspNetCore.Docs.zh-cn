...
对于最小型的 Hello World 类型的应用，每个电路使用约 250 KB 的内存。电路大小取决于应用代码以及与每个组件相关的状态维护要求。我们建议在开发应用和基础设施的过程中衡量资源需求，不过可以将以下基线作为规划部署目标的起点：如果希望应用程序支持 5,000 位并发用户，请考虑为应用预留至少 1.3 GB 服务器内存（或每位用户约为 273 KB）。
