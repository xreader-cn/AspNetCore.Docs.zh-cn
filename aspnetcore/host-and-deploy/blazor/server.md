...
每个路由使用约 250 KB 的内存来实现至少为 Hello World 样式的应用。 每个路由占用内存大小取决于应用代码和与每个组件相关的状态维护要求。 我们建议你在开发应用和基础设施的过程中衡量资源需求，但在计划部署目标时可以将以下基准作为起点：如果希望应用支持 5,000 个并发用户，请考虑为应用预留至少 1.3 GB 服务器内存（或每用户 ~273 KB）。
