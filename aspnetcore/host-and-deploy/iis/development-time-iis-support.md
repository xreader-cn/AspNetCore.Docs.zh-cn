...
1. 在 Windows 中，依次转到“控制面板” > “程序” > “程序和功能” > “启用或禁用 Windows 功能”（位于屏幕左侧）。
...
对于要求使用 HTTPS 的现有项目，使用 `Startup.Configure` 中的 HTTPS 重定向和 HSTS 中间件。 有关更多信息，请参见<xref:security/enforcing-ssl>。
...
如果未使用 Visual Studio，请将启动配置文件手动添加到“属性”文件夹中的 [launchSettings.json](http://json.schemastore.org/launchsettings) 文件内。 下面的示例展示了如何将配置文件配置为使用 HTTPS 协议：
