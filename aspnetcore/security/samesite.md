...
* 默认情况下，不带 SameSite `SameSite=Lax`标头的 cookie 被视为。
* `SameSite=None`必须使用以允许跨站点 cookie。
* 断言`SameSite=None`还必须标记为`Secure`的 cookie。
* 使用[`<iframe>`](https://developer.mozilla.org/docs/Web/HTML/Element/iframe)的应用程序可能会遇到`sameSite=Lax`或`sameSite=Strict` cookie 的`<iframe>`问题，因为被视为跨站点方案。
* [2016 标准](https://tools.ietf.org/html/draft-west-first-party-cookies-07)不允许该值`SameSite=Strict` `SameSite=None` ，并导致某些实现将此类 cookie 视为。 请参阅本文档中的[支持旧版浏览器](#sob)。
...
由于2.2 年12月2019更新发布，.NET Core 支持 SameSite 的2019草案标准。 开发人员能够使用`HttpCookie.SameSite`属性以编程方式控制 sameSite 属性的值。 将`SameSite`属性设置为 Strict、宽松或 None 会导致这些值用 cookie 写入网络。 如果将其设置为（SameSiteMode）（-1），则表示网络上不应包含 cookie 为的 sameSite 属性

[!code-csharp[](samesite/snippets/Privacy.cshtml.cs?name=snippet)]

.NET Core 3.0 支持更新后的 SameSite 值，并向`SameSiteMode.Unspecified` `SameSiteMode`枚举添加额外的枚举值。
...
.NET Framework 和 .NET Core 2.1 的特定行为更改是`SameSite`属性解释`None`值的方式。 在修补程序的值`None`为 "根本不发出属性" 之前，修补后表示 "发出值为的`None`属性"。 修补后，的`SameSite`值将`(SameSiteMode)(-1)`导致不发出属性。

Forms 身份验证和会话状态 cookie 的默认 SameSite 值从`None`更改为。 `Lax`
...
* 重新定义要发出`SameSiteMode.None`的的行为`SameSite=None`
...
SameSite 支持在2.0 中第一次 ASP.NET Core 实现，使用[2016 草案标准](https://tools.ietf.org/html/draft-west-first-party-cookies-07#section-4.1)。 2016标准已选择加入。 默认情况下，ASP.NET Core 选择加入几个`Lax` cookie。 在遇到身份验证的几个[问题](https://github.com/aspnet/Announcements/issues/318)后，大多数 SameSite 使用已[禁用](https://github.com/aspnet/Announcements/issues/348)。
...
* 指定显式断言`SameSite=None`以便启用跨站点传递的 cookie 应标记为`Secure`。 `None`要选择退出的新项。
...
2016 SameSite 标准规定，未知值必须被视为`SameSite=Strict`值。 从支持 2016 SameSite 标准的旧版浏览器访问的应用在收到值为的`None`SameSite 属性时可能会中断。 如果 Web 应用要支持较旧的浏览器，则必须实现浏览器检测。 ASP.NET Core 不会实现浏览器检测，因为用户代理值非常稳定且频繁更改。 中<xref:Microsoft.AspNetCore.CookiePolicy>的扩展点允许插入特定于用户代理的逻辑。

在`Startup.Configure`中，添加在调用<xref:Microsoft.AspNetCore.Builder.CookiePolicyAppBuilderExtensions.UseCookiePolicy*> <xref:Microsoft.AspNetCore.Builder.AuthAppBuilderExtensions.UseAuthentication*>之前调用的代码或*任何*写入 cookie 的方法：
...
在前面的示例中`MyUserAgentDetectionLib.DisallowsSameSiteNone` ，是一个用户提供的库，用于检测用户代理是否不`None`支持 SameSite：

[!code-csharp[](samesite/sample/Startup31.cs?name=snippet2)]

下面的代码演示了一个`DisallowsSameSiteNone`示例方法：
...
Chrome 78 + 提供了令人误解的结果，因为它具有临时的缓解措施。 Chrome 78 + 暂时缓解功能允许 cookie 不到两分钟。 已启用适当测试标志的 Chrome 76 或77提供更准确的结果。 若要测试新的 SameSite 行为`chrome://flags/#same-site-by-default-cookies`切换为**启用状态**。 旧版本的 Chrome （75及更低版本）将报告为失败， `None`并出现新设置。 请参阅本文档中的[支持旧版浏览器](#sob)。
...
从 "未完成`80.0.3975.0`" 的版本开始，可以使用新标志`--enable-features=SameSiteDefaultChecksMethodRigorously`来禁用宽松的 + 后续暂时缓解，以允许在删除缓解功能的最终状态下测试站点和服务。 有关详细信息，请参阅 Chromium 项目[SameSite Updates](https://www.chromium.org/updates/same-site)
...
可以在版本 68 + 上测试对新标准的 Firefox 支持，方法是在`about:config`页面上选择功能标志。 `network.cookie.sameSite.laxByDefault` 以前版本的 Firefox 没有出现兼容性问题的报告。
...
`edge://flags/#same-site-by-default-cookies`页面上设置了 SameSite 标志。 未发现边缘 Chromium 的兼容性问题。
...
| 示例               | 文档 |
| ----------------- | ------------ |
| [.NET Core Razor页](https://github.com/blowdart/AspNetSameSiteSamples/tree/master/AspNetCore31RazorPages)  | <xref:security/samesite/rp31> |
