...
.NET Core 3.0 支持更新后的 SameSite 值，并将额外的枚举值 `SameSiteMode.Unspecified` 添加到 `SameSiteMode` 枚举。
...
.NET Framework 和 .NET Core 2.1 的特定行为更改是 `SameSite` 属性解释 `None` 值的方式。 在修补程序的值 `None` 表示 "根本不发出属性" 之后，修补程序表示 "发出值为 `None`的属性"。 修补后 `SameSite` 值为 `(SameSiteMode)(-1)` 会导致不发出属性。
