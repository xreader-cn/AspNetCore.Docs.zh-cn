...
* `AuthenticationType`设置为显式或`Identity.Application`默认设置为。
...
如果在没有 ASP.NET Core Identity的情况下直接使用 cookie，请在`Startup.ConfigureServices`中配置数据保护和身份验证。 在下面的示例中，"身份验证类型" `Identity.Application`设置为：
...
身份验证 cookie 使用[HttpRequest. PathBase](xref:Microsoft.AspNetCore.Http.HttpRequest.PathBase)作为其默认的[cookie。](xref:Microsoft.AspNetCore.Http.CookieBuilder.Path) 如果应用的 cookie 必须在不同的基路径间共享， `Path`则必须重写：
...
承载跨子域共享 cookie 的应用时，请在 " [Cookie](xref:Microsoft.AspNetCore.Http.CookieBuilder.Domain) " 属性中指定一个公共域。 若要`contoso.com`在中的应用之间共享 cookie `first_subdomain.contoso.com` （ `second_subdomain.contoso.com`例如和） `Cookie.Domain` ， `.contoso.com`请将指定为：
...
生成用户标识时，身份验证`Identity.Application`类型（）必须与在*App_Start/startup.auth.cs* `UseCookieAuthentication`中的`AuthenticationType` "设置" 中定义的类型匹配。
...
当应用使用相同Identity的Identity架构（同一版本）时，请确认每Identity个应用的系统指向同一用户数据库。 否则，当标识系统尝试将身份验证 cookie 中的信息与数据库中的信息进行匹配时，它将在运行时生成故障。

如果Identity架构在应用中不同，通常是因为应用使用的是Identity不同的版本，则在不重新映射和添加其他Identity应用的Identity架构中的列的情况下，不能使用来共享基于最新版本的公共数据库。 将其他应用程序升级到使用最新Identity版本通常更为有效，以便应用可以共享公共数据库。
