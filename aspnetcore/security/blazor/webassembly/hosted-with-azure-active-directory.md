...
按照[快速入门：向 Microsoft 标识平台注册应用程序](/azure/active-directory/develop/quickstart-register-app)和后续 Azure AAD 主题中的指导，在 Azure 门户的**Azure Active Directory** > **应用注册**"区域中为*服务器 API 应用*注册 AAD 应用：

1. 选择“新注册”。 
1. 提供应用的**名称**（例如， ** Blazor服务器 AAD**）。
...
按照[快速入门：向 Microsoft 标识平台注册应用程序](/azure/active-directory/develop/quickstart-register-app)和后续 Azure AAD 主题中的指导，在 Azure 门户的**Azure Active Directory** > **应用注册**"区域中为*客户端应用程序*注册 AAD 应用程序：
...
提供对 ASP.NET Core Web Api 的身份验证和授权的支持是由提供`Microsoft.AspNetCore.Authentication.AzureAD.UI`的：
