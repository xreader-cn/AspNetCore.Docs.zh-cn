...
使用 Open ID Connect （OIDC）提供对独立应用程序的身份验证支持。 `AddOidcAuthentication` 方法接受回调来配置使用 OIDC 对应用进行身份验证所需的参数。 可以从 IP （如 Google、Microsoft 或其他与 OIDC 兼容的提供程序）获取配置应用所需的值。 注册应用时获取值，此操作通常发生在其联机门户中。
