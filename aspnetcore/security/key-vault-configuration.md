...
> [!NOTE]
> 撰写本文时，@no__t 的最新稳定版本 `1.0.3` 版本提供对[系统分配的托管标识](/azure/active-directory/managed-identities-azure-resources/overview#how-does-the-managed-identities-for-azure-resources-work)的支持。 @No__t-1 包中提供了对*用户分配的托管标识*的支持。 本主题演示如何使用系统管理的标识，提供的示例应用使用 @no__t 版本 `Microsoft.Azure.Services.AppAuthentication` 包的版本。

## <a name="sample-app"></a>示例应用

该示例应用程序在*Program.cs*文件顶部 `#define` 语句确定的两种模式中运行：
...
@No__t 提供的说明：此处汇总了使用 Azure CLI @ no__t-0 主题 Azure Key Vault 的机密，用于创建 Azure Key Vault 并存储该示例应用使用的机密。 有关更多详细信息，请参阅主题。
...
1. 在密钥保管库中，以名称-值对的形式创建机密。
   ...
   以下机密用于示例应用。 这些值包含 `_prod` 后缀，以将其与从用户机密的开发环境中加载的 @no__t 1 个后缀值区分开来。 将 `{KEY VAULT NAME}` 替换为在上一步中创建的密钥保管库的名称：
...
当*Program.cs*文件顶部 `#define` 语句设置为 `Certificate` 时，示例应用使用应用程序 ID 和 x.509 证书。
...
@No__t-0 示例应用从 `IConfigurationRoot` 获取其配置值，名称与机密名称相同：

* 非分层值：@No__t-0 的值是通过 @no__t 获取的。
* 分层值（节）：使用 `:` （冒号）表示法或 `GetSection` 扩展方法。 使用以下任一方法来获取配置值：
  * `config["Section:SecretName"]`
  * `config.GetSection("Section")["SecretName"]`

X.509 证书由操作系统管理。 此应用使用*appsettings*文件提供的值调用 `AddAzureKeyVault`：
...
运行应用时，网页将显示加载的机密值。 在开发环境中，用 `_dev` 后缀加载机密值。 在生产环境中，值用 `_prod` 后缀加载。
...
当*Program.cs*文件顶部的 @no__t 0 语句设置为 `Managed` 时，示例应用使用 Azure 资源的托管标识。
...
使用 Azure CLI 和应用程序的对象 ID，为应用提供 @no__t 0 和 @no__t 1 权限，以访问密钥保管库：
...
* 创建不带连接字符串的 @no__t 0 类的实例。 如果未提供连接字符串，提供程序将尝试从 Azure 资源的托管标识获取访问令牌。
* 使用 @no__t 实例令牌回调创建新的 `KeyVaultClient`。
* @No__t-0 实例与 `IKeyVaultSecretManager` 的默认实现一起使用，该实现加载所有机密值并将双引号（`--`）替换为键名称中的冒号（`:`）。

[!code-csharp[](key-vault-configuration/sample/Program.cs?name=snippet2&highlight=13-21)]

运行应用时，网页将显示加载的机密值。 在开发环境中，机密值具有 `_dev` 后缀，因为用户密码提供了这些值。 在生产环境中，值使用 `_prod` 后缀加载，因为它们由 Azure Key Vault 提供。

如果收到 @no__t 错误，请确认已向注册应用程序 Azure AD 并提供对密钥保管库的访问权限。 确认已在 Azure 中重新启动该服务。

## <a name="use-a-key-name-prefix"></a>使用密钥名称前缀

@no__t 提供了一个重载，该重载接受 @no__t 的实现，该实现允许你控制密钥保管库机密如何转换为配置密钥。 例如，你可以实现接口，以便基于在应用启动时提供的前缀值加载机密值。 例如，你可以根据应用程序的版本加载机密。

> [!WARNING]
> 不要对 key vault 机密使用前缀，将多个应用的机密放入同一密钥保管库，或将环境机密（例如，*开发*与*生产*机密）放入同一保管库中。 建议不同的应用和开发/生产环境使用不同的密钥保管库，以便隔离应用环境以获得最高级别的安全性。

在下面的示例中，在密钥保管库中建立了机密（并使用开发环境的机密管理器工具） `5000-AppSecret` （密钥保管库机密名称中不允许使用句点）。 此机密代表应用程序版本5.0.0.0 的应用程序机密。 对于其他版本的应用，5.1.0.0，会将机密添加到密钥保管库（并使用机密管理器工具） `5100-AppSecret`。 每个应用版本会将其版本控制的机密值加载到 `AppSecret` 的配置中，并在加载机密时将版本去除。

使用自定义 @no__t @no__t 调用-0：

[!code-csharp[](key-vault-configuration/sample_snapshot/Program.cs?highlight=30-34)]

@No__t 0 实现可对机密的版本前缀做出反应，以将适当的机密加载到配置中：

[!code-csharp[](key-vault-configuration/sample_snapshot/Startup.cs?name=snippet1)]

@No__t-0 方法由提供程序算法调用，该算法将循环访问保管库机密以查找具有版本前缀的文件。 当使用 `Load` 找到版本前缀时，算法使用 `GetKey` 方法返回密钥名称的配置名称。 它从机密名称中去除版本前缀，并返回密钥名称的其余部分，以便加载到应用的配置名称-值对中。
...
1. 当应用运行时，将加载密钥保管库机密。 @No__t 的字符串机密与应用程序的项目文件（@no__t 为-1）中指定的应用程序版本相匹配。

1. 将从键名称中去除版本 `5000` （带有短划线）。 在整个应用程序中，读取带有密钥 @no__t 的配置-0 会加载机密值。

1. 如果在项目文件中将应用程序的版本更改为 `5.1.0.0` 并再次运行应用程序，则返回的机密值在开发环境中 @no__t 为-1，在生产环境中 `5.1.0.0_secret_value_prod`。

> [!NOTE]
> 你还可以提供自己的 @no__t 0 实现来 `AddAzureKeyVault`。 自定义客户端允许跨应用共享客户端的单个实例。

## <a name="bind-an-array-to-a-class"></a>将数组绑定至类

提供程序能够将配置值读入数组，以便绑定到 POCO 数组。

从允许键包含冒号（`:`）分隔符的配置源中进行读取时，将使用数字键段来区分组成数组的键（`:0:`，`:1:` 。 `:{n}:`）格式模式中出现的位置生成。 有关详细信息，请参阅 [Configuration：将数组绑定到类 @ no__t-0。

Azure Key Vault 密钥不能使用冒号作为分隔符。 本主题中所述的方法使用双短划线（`--`）作为层次结构值（节）的分隔符。 数组键存储在 Azure Key Vault 中，具有双短划线和数字键段（`--0--`，`--1--`，&hellip; @no__t）。

检查 JSON 文件提供的以下[Serilog](https://serilog.net/)日志记录提供程序配置。 @No__t-0 数组中定义了两个对象文本，该数组反映了两个 Serilog*接收器*，它们描述了日志记录输出的目标：
...
在调用 `IConfigurationRoot.Reload()` 之前会缓存机密。 在执行 `Reload` 之前，应用程序不会遵守密钥保管库中的已过期、已禁用和更新的机密。
...
禁用和过期的机密会在运行时引发 `KeyVaultClientException`。 若要防止应用程序引发，请使用不同的配置提供程序提供配置，或更新禁用或过期的机密。
...
* 应用无权访问密钥保管库。
* 访问策略不包括 @no__t 0 和 @no__t 权限。
* 在密钥保管库中，配置数据（名称-值对）被错误地命名、缺失、禁用或过期。
* 应用具有错误的密钥保管库名称（`KeyVaultName`）、Azure AD 应用程序 Id （@no__t）或 Azure AD 证书指纹（`AzureADCertThumbprint`）。
* 在应用程序中，配置项（名称）的值不正确。
...
* @no__t 0Microsoft Azure：Key Vault 文档 @ no__t-0
...
* [快速入门：使用 .NET web 应用 @ no__t 设置和检索 Azure Key Vault 的机密-0
* [教程：如何在中将 Azure Key Vault 与 Azure Windows 虚拟机一起使用。NET @ no__t-0
