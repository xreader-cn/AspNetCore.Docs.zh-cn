...
X.509 证书由操作系统管理。 应用使用*appsettings*文件提供的值调用 `AddAzureKeyVault`：
...
使用自定义 `IKeyVaultSecretManager` 调用 `AddAzureKeyVault`：
...
> [!NOTE]
> 你还可以提供自己的 `KeyVaultClient` 实现以 `AddAzureKeyVault`。 自定义客户端允许跨应用共享客户端的单个实例。
