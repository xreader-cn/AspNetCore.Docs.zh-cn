...
1. 使用[Azure 门户](https://portal.azure.com/)中的以下方法之一打开 Azure Cloud shell:
   ...
   * 选择 Azure 门户右上角菜单中的 " **Cloud Shell** " 按钮。
   ...
   有关详细信息, 请参阅[Azure 命令行接口 (CLI)](/cli/azure/)和[Azure Cloud Shell 的概述](/azure/cloud-shell/overview)。
...
当从允许键包含冒号 (`:`) 分隔符的配置源中进行读取时, 将使用数字键段来区分组成数组的键 (`:0:`, `:1:`, 。 `:{n}:`）格式模式中出现的位置生成。 有关详细信息, 请[参阅配置:将数组绑定到类](xref:fundamentals/configuration/index#bind-an-array-to-a-class)。
