...
在`Startup.Configure`中， `UseAuthentication`调用`UseAuthorization`并设置`HttpContext.User`属性并为请求运行授权中间件。 在调用`UseAuthentication` `UseEndpoints`之前`UseAuthorization`调用和方法：

[!code-csharp[](cookie/samples/3.x/CookieSample/Startup.cs?name=snippet2)]

<xref:Microsoft.AspNetCore.Authentication.Cookies.CookieAuthenticationOptions>类用于配置身份验证提供程序选项。

在`CookieAuthenticationOptions`服务配置中设置，以便在`Startup.ConfigureServices`方法中进行身份验证：
...
默认<xref:Microsoft.AspNetCore.Builder.CookiePolicyOptions.MinimumSameSitePolicy>值`SameSiteMode.Lax`为允许 OAuth2 authentication。 若要严格地强制执行同一站点策略`SameSiteMode.Strict`，请设置`MinimumSameSitePolicy`。 尽管此设置会中断 OAuth2 和其他跨源身份验证方案，但它为不依赖于跨源请求处理的其他类型的应用提升了 cookie 安全级别。
...
<xref:System.Security.Claims.ClaimsIdentity>使用任何所需<xref:System.Security.Claims.Claim>的和调用<xref:Microsoft.AspNetCore.Authentication.AuthenticationHttpContextExtensions.SignInAsync*>来登录用户：
...
若要在数据库基于`LastChanged`值更改时使 cookie 无效，请使用包含数据库中的当前`LastChanged` `LastChanged`值的声明创建 cookie：
...
请考虑这样一种情况：用户的名称更新&mdash;为不影响安全的决策。 如果要以非破坏性的更新用户主体，请调用`context.ReplacePrincipal` ，并将`context.ShouldRenew`属性设置`true`为。
...
绝对过期时间可以设置为<xref:Microsoft.AspNetCore.Authentication.AuthenticationProperties.ExpiresUtc>。 若要创建持久性 cookie， `IsPersistent`还必须设置。 否则，cookie 是使用基于会话的生存期创建的，并且可能会在它所包含的身份验证票证之前或之后过期。 设置`ExpiresUtc`后，它将覆盖的<xref:Microsoft.AspNetCore.Authentication.Cookies.CookieAuthenticationOptions.ExpireTimeSpan>选项的<xref:Microsoft.AspNetCore.Authentication.Cookies.CookieAuthenticationOptions>值（如果已设置）。
...
在`Startup.Configure`方法中，调用`UseAuthentication`方法以调用设置`HttpContext.User`属性的身份验证中间件。 在调用`UseAuthentication` `UseMvcWithDefaultRoute`或`UseMvc`之前调用方法：
