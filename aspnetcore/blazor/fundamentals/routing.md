...
| `long`     | `{ticks:long}`    | <span data-ttu-id="97a0a-173">`123456789`, `-123456789`</span><span class="sxs-lookup"><span data-stu-id="97a0a-173">`123456789`, `-123456789`</span></span>                                                        | <span data-ttu-id="97a0a-174">是</span><span class="sxs-lookup"><span data-stu-id="97a0a-174">Yes</span></span>                              |
...
| <span data-ttu-id="97a0a-219">成员</span><span class="sxs-lookup"><span data-stu-id="97a0a-219">Member</span></span> | <span data-ttu-id="97a0a-220">描述</span><span class="sxs-lookup"><span data-stu-id="97a0a-220">Description</span></span> |
| ------ | ----------- |
| <xref:Microsoft.AspNetCore.Components.NavigationManager.Uri> | <span data-ttu-id="97a0a-221">获取当前绝对 URI。</span><span class="sxs-lookup"><span data-stu-id="97a0a-221">Gets the current absolute URI.</span></span> |
| <xref:Microsoft.AspNetCore.Components.NavigationManager.BaseUri> | <span data-ttu-id="97a0a-222">获取可在相对 URI 路径之前添加用于生成绝对 URI 的基 URI（带有尾部反斜杠）。</span><span class="sxs-lookup"><span data-stu-id="97a0a-222">Gets the base URI (with a trailing slash) that can be prepended to relative URI paths to produce an absolute URI.</span></span> <span data-ttu-id="97a0a-223">通常，<xref:Microsoft.AspNetCore.Components.NavigationManager.BaseUri> 对应于 `wwwroot/index.html` (Blazor WebAssembly) 或 `Pages/_Host.cshtml` (Blazor Server) 中文档的 `<base>` 元素上的 `href` 属性。</span><span class="sxs-lookup"><span data-stu-id="97a0a-223">Typically, <xref:Microsoft.AspNetCore.Components.NavigationManager.BaseUri> corresponds to the `href` attribute on the document's `<base>` element in `wwwroot/index.html` (Blazor WebAssembly) or `Pages/_Host.cshtml` (Blazor Server).</span></span> |
| <xref:Microsoft.AspNetCore.Components.NavigationManager.NavigateTo%2A> | <span data-ttu-id="97a0a-224">导航到指定 URI。</span><span class="sxs-lookup"><span data-stu-id="97a0a-224">Navigates to the specified URI.</span></span> <span data-ttu-id="97a0a-225">如果 `forceLoad` 为 `true`，则：</span><span class="sxs-lookup"><span data-stu-id="97a0a-225">If `forceLoad` is `true`:</span></span><ul><li><span data-ttu-id="97a0a-226">客户端路由会被绕过。</span><span class="sxs-lookup"><span data-stu-id="97a0a-226">Client-side routing is bypassed.</span></span></li><li><span data-ttu-id="97a0a-227">无论 URI 是否通常由客户端路由器处理，浏览器都必须从服务器加载新页面。</span><span class="sxs-lookup"><span data-stu-id="97a0a-227">The browser is forced to load the new page from the server, whether or not the URI is normally handled by the client-side router.</span></span></li></ul> |
| <xref:Microsoft.AspNetCore.Components.NavigationManager.LocationChanged> | <span data-ttu-id="97a0a-228">导航位置更改时触发的事件。</span><span class="sxs-lookup"><span data-stu-id="97a0a-228">An event that fires when the navigation location has changed.</span></span> |
| <xref:Microsoft.AspNetCore.Components.NavigationManager.ToAbsoluteUri%2A> | <span data-ttu-id="97a0a-229">将相对 URI 转换为绝对 URI。</span><span class="sxs-lookup"><span data-stu-id="97a0a-229">Converts a relative URI into an absolute URI.</span></span> |
| <span style="word-break:normal;word-wrap:normal"><xref:Microsoft.AspNetCore.Components.NavigationManager.ToBaseRelativePath%2A></span> | <span data-ttu-id="97a0a-230">给定基 URI（例如，之前由 <xref:Microsoft.AspNetCore.Components.NavigationManager.BaseUri> 返回的 URI），将绝对 URI 转换为相对于基 URI 前缀的 URI。</span><span class="sxs-lookup"><span data-stu-id="97a0a-230">Given a base URI (for example, a URI previously returned by <xref:Microsoft.AspNetCore.Components.NavigationManager.BaseUri>), converts an absolute URI into a URI relative to the base URI prefix.</span></span> |

<span data-ttu-id="97a0a-231">选择该按钮后，以下组件导航到应用的 `Counter` 组件：</span><span class="sxs-lookup"><span data-stu-id="97a0a-231">The following component navigates to the app's `Counter` component when the button is selected:</span></span>
...
<span data-ttu-id="97a0a-232">以下组件通过订阅 <xref:Microsoft.AspNetCore.Components.NavigationManager.LocationChanged?displayProperty=nameWithType> 来处理位置改变事件。</span><span class="sxs-lookup"><span data-stu-id="97a0a-232">The following component handles a location changed event by subscribing to <xref:Microsoft.AspNetCore.Components.NavigationManager.LocationChanged?displayProperty=nameWithType>.</span></span> <span data-ttu-id="97a0a-233">在框架调用 `Dispose` 时，解除挂接 `HandleLocationChanged` 方法。</span><span class="sxs-lookup"><span data-stu-id="97a0a-233">The `HandleLocationChanged` method is unhooked when `Dispose` is called by the framework.</span></span> <span data-ttu-id="97a0a-234">解除挂接该方法可允许组件进行垃圾回收。</span><span class="sxs-lookup"><span data-stu-id="97a0a-234">Unhooking the method permits garbage collection of the component.</span></span>
