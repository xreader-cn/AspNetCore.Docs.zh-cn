...
<span data-ttu-id="cd1b8-108">支持以下部署策略：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-108">The following deployment strategies are supported:</span></span>
...
<span data-ttu-id="cd1b8-147">由于浏览器针对客户端页面请求基于 Internet 的主机，因此 Web 服务器和托管服务必须将对服务器上非物理方式资源的所有请求重写为 `index.html` 页。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-147">Because browsers make requests to Internet-based hosts for client-side pages, web servers and hosting services must rewrite all requests for resources not physically on the server to the `index.html` page.</span></span> <span data-ttu-id="cd1b8-148">如果返回 `index.html`，应用的 Blazor 路由器将接管工作并使用正确的资源响应。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-148">When `index.html` is returned, the app's Blazor router takes over and responds with the correct resource.</span></span>
...
### <a name="app-configuration"></a><span data-ttu-id="cd1b8-160">应用配置</span><span class="sxs-lookup"><span data-stu-id="cd1b8-160">App configuration</span></span>
...
* <span data-ttu-id="cd1b8-171">向解决方案添加第二个客户端应用：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-171">Add a second client app to the solution:</span></span>
  ...
  * <span data-ttu-id="cd1b8-172">向解决方案的文件夹添加一个名为 `SecondClient` 的文件夹。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-172">Add a folder named `SecondClient` to the solution's folder.</span></span> <span data-ttu-id="cd1b8-173">在添加 `SecondClient` 文件夹后，通过项目模板创建的解决方案文件夹包含以下解决方案文件和文件夹：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-173">The solution folder created from the project template contains the following solution file and folders after the `SecondClient` folder is added:</span></span>
    ...
    * <span data-ttu-id="cd1b8-174">`Client`（文件夹）</span><span class="sxs-lookup"><span data-stu-id="cd1b8-174">`Client` (folder)</span></span>
...
* <span data-ttu-id="cd1b8-185">在服务器应用的项目文件中，为新增的 `SecondBlazorApp.Client` 客户端应用创建一个项目引用：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-185">In the server app's project file, create a project reference for the added `SecondBlazorApp.Client` client app:</span></span>
  ...
  <span data-ttu-id="cd1b8-186">占位符 `{SOLUTION NAME}` 是解决方案的名称。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-186">The placeholder `{SOLUTION NAME}` is the solution's name.</span></span>
...
* <span data-ttu-id="cd1b8-188">在服务器应用的 `Startup.Configure` 方法 (`Startup.cs`) 中，删除在调用 <xref:Microsoft.AspNetCore.Builder.HttpsPolicyBuilderExtensions.UseHttpsRedirection%2A> 后显示的以下行：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-188">In the server app's `Startup.Configure` method (`Startup.cs`), remove the following lines, which appear after the call to <xref:Microsoft.AspNetCore.Builder.HttpsPolicyBuilderExtensions.UseHttpsRedirection%2A>:</span></span>
  ...
  * <span data-ttu-id="cd1b8-192">原始客户端应用的请求主机为 `firstapp.com`，或新增的客户端应用的请求主机为 `secondapp.com`。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-192">The request host is either `firstapp.com` for the original client app or `secondapp.com` for the added client app.</span></span>
  ...
  <span data-ttu-id="cd1b8-198">将以下代码放在先前已删除行的位置：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-198">Place the following code where the lines were removed earlier:</span></span>
...
* <span data-ttu-id="cd1b8-211">来自客户端应用的 `wwwroot` 文件夹中的图像 (`jeep-cj.png`)。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-211">An image from the client app's `wwwroot` folder (`jeep-cj.png`).</span></span>
...
<span data-ttu-id="cd1b8-217">客户端应用的 `wwwroot/index.html` 文件要求库的样式表添加以下 `<link>` 标记：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-217">The client app's `wwwroot/index.html` file requests the library's stylesheet with the following added `<link>` tag:</span></span>
...
### <a name="iis"></a><span data-ttu-id="cd1b8-229">IIS</span><span class="sxs-lookup"><span data-stu-id="cd1b8-229">IIS</span></span>
...
* <span data-ttu-id="cd1b8-264">`web.config` 文件，IIS 使用该文件配置网站，包括所需的重定向规则和文件内容类型。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-264">The `web.config` file that IIS uses to configure the website, including the required redirect rules and file content types.</span></span>
...
#### <a name="brotli-and-gzip-compression"></a><span data-ttu-id="cd1b8-273">Brotli 和 Gzip 压缩</span><span class="sxs-lookup"><span data-stu-id="cd1b8-273">Brotli and Gzip compression</span></span>
...
### <a name="azure-storage"></a><span data-ttu-id="cd1b8-288">Azure 存储</span><span class="sxs-lookup"><span data-stu-id="cd1b8-288">Azure Storage</span></span>
...
### <a name="nginx-in-docker"></a><span data-ttu-id="cd1b8-310">Docker 中的 Nginx</span><span class="sxs-lookup"><span data-stu-id="cd1b8-310">Nginx in Docker</span></span>
...
1. <span data-ttu-id="cd1b8-318">将 Apache 配置文件放入 `/etc/httpd/conf.d/` 目录（这是 CentOS 7 中的默认 Apache 配置目录）。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-318">Place the Apache configuration file into the `/etc/httpd/conf.d/` directory, which is the default Apache configuration directory in CentOS 7.</span></span>
...
## <a name="host-configuration-values"></a><span data-ttu-id="cd1b8-330">主机配置值</span><span class="sxs-lookup"><span data-stu-id="cd1b8-330">Host configuration values</span></span>
...
* <span data-ttu-id="cd1b8-382">URI 字符串。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-382">URI string.</span></span> <span data-ttu-id="cd1b8-383">在以下示例 (`wwwroot/index.html`) 中，以下文件来自 CDN (`https://my-awesome-cdn.com/`)：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-383">In the following example (`wwwroot/index.html`), the following files are served from a CDN at `https://my-awesome-cdn.com/`:</span></span>

  * `dotnet.*.js`
  * `dotnet.wasm`
  * <span data-ttu-id="cd1b8-384">时区数据</span><span class="sxs-lookup"><span data-stu-id="cd1b8-384">Timezone data</span></span>
...
* <span data-ttu-id="cd1b8-385">`Promise<Response>`.</span><span class="sxs-lookup"><span data-stu-id="cd1b8-385">`Promise<Response>`.</span></span> <span data-ttu-id="cd1b8-386">在标头中传递 `integrity` 参数以保持默认的完整性检查行为。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-386">Pass the `integrity` parameter in a header to retain the default integrity-checking behavior.</span></span>

  <span data-ttu-id="cd1b8-387">以下示例 (`wwwroot/index.html`) 将一个自定义 HTTP 标头添加到出站请求，并将 `integrity` 参数传递到 `fetch` 调用：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-387">The following example (`wwwroot/index.html`) adds a custom HTTP header to the outbound requests and passes the `integrity` parameter through to the `fetch` call:</span></span>
...
* <span data-ttu-id="cd1b8-388">`null`/`undefined`，这将导致默认的加载行为。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-388">`null`/`undefined`, which results in the default loading behavior.</span></span>

<span data-ttu-id="cd1b8-389">外部源必须返回浏览器所需的 CORS 标头以允许跨域资源加载。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-389">External sources must return the required CORS headers for browsers to allow the cross-origin resource loading.</span></span> <span data-ttu-id="cd1b8-390">CDN 通常会默认提供所需的标头。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-390">CDNs usually provide the required headers by default.</span></span>

<span data-ttu-id="cd1b8-391">你只需指定自定义行为的类型。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-391">You only need to specify types for custom behaviors.</span></span> <span data-ttu-id="cd1b8-392">框架会根据默认的加载行为加载未指定到 `loadBootResource` 的类型。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-392">Types not specified to `loadBootResource` are loaded by the framework per their default loading behaviors.</span></span>

## <a name="change-the-filename-extension-of-dll-files"></a><span data-ttu-id="cd1b8-393">更改 DLL 文件的文件扩展名</span><span class="sxs-lookup"><span data-stu-id="cd1b8-393">Change the filename extension of DLL files</span></span>

<span data-ttu-id="cd1b8-394">如果需要更改应用的已发布 `.dll` 文件的文件扩展名，请按照本部分中的指导进行操作。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-394">In case you have a need to change the filename extensions of the app's published `.dll` files, follow the guidance in this section.</span></span>

<span data-ttu-id="cd1b8-395">发布应用后，使用 shell 脚本或 DevOps 生成管道将 `.dll` 文件重命名，以使用其他文件扩展名。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-395">After publishing the app, use a shell script or DevOps build pipeline to rename `.dll` files to use a different file extension.</span></span> <span data-ttu-id="cd1b8-396">将 `.dll` 文件的目标位置设为应用的已发布输出的 `wwwroot` 目录中（例如 `{CONTENT ROOT}/bin/Release/netstandard2.1/publish/wwwroot`）。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-396">Target the `.dll` files in the `wwwroot` directory of the app's published output (for example, `{CONTENT ROOT}/bin/Release/netstandard2.1/publish/wwwroot`).</span></span>

<span data-ttu-id="cd1b8-397">在下面的示例中，重命名 `.dll` 文件，以使用 `.bin` 文件扩展名。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-397">In the following examples, `.dll` files are renamed to use the `.bin` file extension.</span></span>

<span data-ttu-id="cd1b8-398">在 Windows 上：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-398">On Windows:</span></span>
...
<span data-ttu-id="cd1b8-399">如果服务工作进程资产也在使用中，请添加以下命令：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-399">If service worker assets are also in use, add the following command:</span></span>

```powershell
((Get-Content .\service-worker-assets.js -Raw) -replace '.dll"','.bin"') | Set-Content .\service-worker-assets.js
```

<span data-ttu-id="cd1b8-400">在 Linux 或 macOS 上：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-400">On Linux or macOS:</span></span>
...
<span data-ttu-id="cd1b8-401">如果服务工作进程资产也在使用中，请添加以下命令：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-401">If service worker assets are also in use, add the following command:</span></span>

```console
sed -i 's/\.dll"/.bin"/g' service-worker-assets.js
```
   
<span data-ttu-id="cd1b8-402">要使用不同于 `.bin` 的其他文件扩展名，请在前面的命令中替换 `.bin`。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-402">To use a different file extension than `.bin`, replace `.bin` in the preceding commands.</span></span>

<span data-ttu-id="cd1b8-403">若要处理压缩的 `blazor.boot.json.gz` 和 `blazor.boot.json.br` 文件，请采用以下方法之一：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-403">To address the compressed `blazor.boot.json.gz` and `blazor.boot.json.br` files, adopt either of the following approaches:</span></span>

* <span data-ttu-id="cd1b8-404">删除压缩的 `blazor.boot.json.gz` 和 `blazor.boot.json.br` 文件。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-404">Remove the compressed `blazor.boot.json.gz` and `blazor.boot.json.br` files.</span></span> <span data-ttu-id="cd1b8-405">此方法禁用压缩。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-405">Compression is disabled with this approach.</span></span>
* <span data-ttu-id="cd1b8-406">重新压缩更新后的 `blazor.boot.json` 文件。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-406">Recompress the updated `blazor.boot.json` file.</span></span>

<span data-ttu-id="cd1b8-407">上述指导也适用于正在使用服务工作进程资产的情况。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-407">The preceding guidance also applies when service worker assets are in use.</span></span> <span data-ttu-id="cd1b8-408">删除或重新压缩 `wwwroot/service-worker-assets.js.br` 和 `wwwroot/service-worker-assets.js.gz`。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-408">Remove or recompress `wwwroot/service-worker-assets.js.br` and `wwwroot/service-worker-assets.js.gz`.</span></span> <span data-ttu-id="cd1b8-409">否则，浏览器中的文件完整性检查将失败。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-409">Otherwise, file integrity checks fail in the browser.</span></span>

<span data-ttu-id="cd1b8-410">以下 Windows 示例使用项目根目录中的 PowerShell 脚本。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-410">The following Windows example uses a PowerShell script placed at the root of the project.</span></span>

<span data-ttu-id="cd1b8-411">`ChangeDLLExtensions.ps1:`:</span><span class="sxs-lookup"><span data-stu-id="cd1b8-411">`ChangeDLLExtensions.ps1:`:</span></span>
...
<span data-ttu-id="cd1b8-412">如果服务工作进程资产也在使用中，请添加以下命令：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-412">If service worker assets are also in use, add the following command:</span></span>

```powershell
((Get-Content $filepath\bin\Release\$tfm\wwwroot\service-worker-assets.js -Raw) -replace '.dll"','.bin"') | Set-Content $filepath\bin\Release\$tfm\wwwroot\service-worker-assets.js
```

<span data-ttu-id="cd1b8-413">在项目文件中，在发布应用后运行脚本：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-413">In the project file, the script is run after publishing the app:</span></span>
...
> [!NOTE]
> <span data-ttu-id="cd1b8-414">重命名和延迟加载相同的程序集时，请参阅 <xref:blazor/webassembly-lazy-load-assemblies#onnavigateasync-events-and-renamed-assembly-files> 中的指南。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-414">When renaming and lazy loading the same assemblies, see the guidance in <xref:blazor/webassembly-lazy-load-assemblies#onnavigateasync-events-and-renamed-assembly-files>.</span></span>

## <a name="resolve-integrity-check-failures"></a><span data-ttu-id="cd1b8-415">解决完整性检查失败</span><span class="sxs-lookup"><span data-stu-id="cd1b8-415">Resolve integrity check failures</span></span>

<span data-ttu-id="cd1b8-416">当 Blazor WebAssembly 下载应用的启动文件时，它会指示浏览器对响应执行完整性检查。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-416">When Blazor WebAssembly downloads an app's startup files, it instructs the browser to perform integrity checks on the responses.</span></span> <span data-ttu-id="cd1b8-417">它使用 `blazor.boot.json` 文件中的信息为 `.dll`、`.wasm` 和其他文件指定预期的 SHA-256 哈希值。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-417">It uses information in the `blazor.boot.json` file to specify the expected SHA-256 hash values for `.dll`, `.wasm`, and other files.</span></span> <span data-ttu-id="cd1b8-418">这对以下原因有所帮助：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-418">This is beneficial for the following reasons:</span></span>

* <span data-ttu-id="cd1b8-419">它可确保不会出现加载不一致文件集的风险，例如，在用户正在下载应用程序文件时将新部署应用到 Web 服务器的情况。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-419">It ensures you don't risk loading an inconsistent set of files, for example if a new deployment is applied to your web server while the user is in the process of downloading the application files.</span></span> <span data-ttu-id="cd1b8-420">不一致的文件可能导致未定义的行为。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-420">Inconsistent files could lead to undefined behavior.</span></span>
* <span data-ttu-id="cd1b8-421">它可确保用户的浏览器从不缓存不一致或无效的响应，这些响应可能会阻止他们启动应用（即使他们手动刷新了页面也是如此）。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-421">It ensures the user's browser never caches inconsistent or invalid responses, which could prevent them from starting the app even if they manually refresh the page.</span></span>
* <span data-ttu-id="cd1b8-422">它可以安全地缓存响应，甚至无需检查服务器端更改，直到预期的 SHA-256 哈希本身发生更改，因此，后续页面加载需要较少的请求即可快速完成。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-422">It makes it safe to cache the responses and not even check for server-side changes until the expected SHA-256 hashes themselves change, so subsequent page loads involve fewer requests and complete much faster.</span></span>

<span data-ttu-id="cd1b8-423">如果你的 Web 服务器返回的响应与预期的 SHA-256 哈希不匹配，你将看到类似于以下内容的错误显示在浏览器的开发人员控制台中：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-423">If your web server returns responses that don't match the expected SHA-256 hashes, you will see an error similar to the following appear in the browser's developer console:</span></span>

> <span data-ttu-id="cd1b8-424">未能使用计算出的 SHA-256 完整性“IIa70iwvmEg5WiDV17OpQ5eCztNYqL186J56852RpJY=”在资源“https://myapp.example.com/\_framework/MyBlazor App.dll”的“integrity”属性中找到有效摘要。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-424">Failed to find a valid digest in the 'integrity' attribute for resource 'https://myapp.example.com/\_framework/MyBlazorApp.dll' with computed SHA-256 integrity 'IIa70iwvmEg5WiDV17OpQ5eCztNYqL186J56852RpJY='.</span></span> <span data-ttu-id="cd1b8-425">资源已被阻止。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-425">The resource has been blocked.</span></span>

<span data-ttu-id="cd1b8-426">在大多数情况下，这不是完整性检查本身的问题。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-426">In most cases, this is *not* a problem with integrity checking itself.</span></span> <span data-ttu-id="cd1b8-427">相反，它表示存在其他问题，并且完整性检查会警告你其他问题。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-427">Instead, it means there is some other problem, and the integrity check is warning you about that other problem.</span></span>

### <a name="diagnosing-integrity-problems"></a><span data-ttu-id="cd1b8-428">诊断完整性问题</span><span class="sxs-lookup"><span data-stu-id="cd1b8-428">Diagnosing integrity problems</span></span>

<span data-ttu-id="cd1b8-429">生成应用时，生成的 `blazor.boot.json` 清单将描述生成输出生成时启动资源（例如，`.dll`、`.wasm` 和其他文件）的 SHA-256 哈希。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-429">When an app is built, the generated `blazor.boot.json` manifest describes the SHA-256 hashes of your boot resources (for example, `.dll`, `.wasm`, and other files) at the time that the build output is produced.</span></span> <span data-ttu-id="cd1b8-430">只要 `blazor.boot.json` 中的 SHA-256 哈希与传递到浏览器的文件相匹配，完整性检查就会通过。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-430">The integrity check passes as long as the SHA-256 hashes in `blazor.boot.json` match the files delivered to the browser.</span></span>

<span data-ttu-id="cd1b8-431">此失败的常见原因包括：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-431">Common reasons why this fails are:</span></span>

 * <span data-ttu-id="cd1b8-432">Web 服务器的响应是一个错误（例如，“404 - 找不到”或“500 - 内部服务器错误”），而不是浏览器所请求的文件。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-432">The web server's response is an error (for example, a *404 - Not Found* or a *500 - Internal Server Error*) instead of the file the browser requested.</span></span> <span data-ttu-id="cd1b8-433">浏览器会将其报告为完整性检查失败，而不是响应失败。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-433">This is reported by the browser as an integrity check failure and not as a response failure.</span></span>
 * <span data-ttu-id="cd1b8-434">在文件生成和传递到浏览器之间已更改文件的内容。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-434">Something has changed the contents of the files between the build and delivery of the files to the browser.</span></span> <span data-ttu-id="cd1b8-435">下面可能会发生这种情况：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-435">This might happen:</span></span>
   * <span data-ttu-id="cd1b8-436">你或生成工具手动修改生成输出的情况。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-436">If you or build tools manually modify the build output.</span></span>
   * <span data-ttu-id="cd1b8-437">部署过程的某个方面修改了文件的情况。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-437">If some aspect of the deployment process modified the files.</span></span> <span data-ttu-id="cd1b8-438">例如，在使用基于 Git 的部署机制时，请记住，如果你在 Windows 上提交文件并在 Linux 上检查它们，则 Git 会以透明方式将 Windows 样式的行尾转换为 Unix 样式的行尾。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-438">For example if you use a Git-based deployment mechanism, bear in mind that Git transparently converts Windows-style line endings to Unix-style line endings if you commit files on Windows and check them out on Linux.</span></span> <span data-ttu-id="cd1b8-439">更改文件行尾将更改 SHA-256 哈希。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-439">Changing file line endings change the SHA-256 hashes.</span></span> <span data-ttu-id="cd1b8-440">若要避免此问题，请考虑[使用 `.gitattributes` 将生成项目视为 `binary` 文件](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes)。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-440">To avoid this problem, consider [using `.gitattributes` to treat build artifacts as `binary` files](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes).</span></span>
   * <span data-ttu-id="cd1b8-441">Web 服务器在提供文件内容的过程中对其进行修改。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-441">The web server modifies the file contents as part of serving them.</span></span> <span data-ttu-id="cd1b8-442">例如，某些内容分发网络 (CDN) 会自动尝试[缩小](xref:client-side/bundling-and-minification#minification) HTML，从而对其进行修改。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-442">For example, some content distribution networks (CDNs) automatically attempt to [minify](xref:client-side/bundling-and-minification#minification) HTML, thereby modifying it.</span></span> <span data-ttu-id="cd1b8-443">可能需要禁用此类功能。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-443">You may need to disable such features.</span></span>

<span data-ttu-id="cd1b8-444">若要诊断哪些功能适用于你的情况，请执行执行操作：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-444">To diagnose which of these applies in your case:</span></span>

 1. <span data-ttu-id="cd1b8-445">通过读取错误消息来记下哪个文件触发错误。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-445">Note which file is triggering the error by reading the error message.</span></span>
 1. <span data-ttu-id="cd1b8-446">打开浏览器的开发人员工具，然后在“网络”选项卡中查找。如有必要，请重新加载页面以查看请求和响应的列表。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-446">Open your browser's developer tools and look in the *Network* tab. If necessary, reload the page to see the list of requests and responses.</span></span> <span data-ttu-id="cd1b8-447">在该列表中查找触发错误的文件。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-447">Find the file that is triggering the error in that list.</span></span>
 1. <span data-ttu-id="cd1b8-448">检查响应中的 HTTP 状态代码。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-448">Check the HTTP status code in the response.</span></span> <span data-ttu-id="cd1b8-449">如果服务器返回除“200 - 正常”（或其他 2xx 状态代码）以外的任何内容，则需要诊断服务器端问题。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-449">If the server returns anything other than *200 - OK* (or another 2xx status code), then you have a server-side problem to diagnose.</span></span> <span data-ttu-id="cd1b8-450">例如，状态代码 403 表示存在授权问题，而状态代码 500 表示服务器以未指定的方式失败。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-450">For example, status code 403 means there's an authorization problem, whereas status code 500 means the server is failing in an unspecified manner.</span></span> <span data-ttu-id="cd1b8-451">请参阅服务器端日志以诊断和修复应用。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-451">Consult server-side logs to diagnose and fix the app.</span></span>
 1. <span data-ttu-id="cd1b8-452">如果资源的状态代码为“200 - 正常”，请在浏览器的开发人员工具中查看响应内容，并检查内容是否与预期的数据匹配。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-452">If the status code is *200 - OK* for the resource, look at the response content in browser's developer tools and check that the content matches up with the data expected.</span></span> <span data-ttu-id="cd1b8-453">例如，常见问题是错误配置了路由，因此请求甚至返回其他文件的 `index.html` 数据。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-453">For example, a common problem is to misconfigure routing so that requests return your `index.html` data even for other files.</span></span> <span data-ttu-id="cd1b8-454">请确保对 `.wasm` 请求的响应是 WebAssembly 二进制文件，对 `.dll` 请求的响应是 .NET 程序集二进制文件。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-454">Make sure that responses to `.wasm` requests are WebAssembly binaries and that responses to `.dll` requests are .NET assembly binaries.</span></span> <span data-ttu-id="cd1b8-455">如果不是，则需要诊断服务器端路由问题。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-455">If not, you have a server-side routing problem to diagnose.</span></span>
 1. <span data-ttu-id="cd1b8-456">搜索以验证应用的已发布和已部署输出，并提供[完整性 PowerShell 脚本故障排除](#troubleshoot-integrity-powershell-script)。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-456">Seek to validate the app's published and deployed output with the [Troubleshoot integrity PowerShell script](#troubleshoot-integrity-powershell-script).</span></span>

<span data-ttu-id="cd1b8-457">如果确认服务器返回看似正确的数据，则必须在生成文件和传递文件之间修改内容。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-457">If you confirm that the server is returning plausibly correct data, there must be something else modifying the contents in between build and delivery of the file.</span></span> <span data-ttu-id="cd1b8-458">若要对此进行调查，请执行以下操作：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-458">To investigate this:</span></span>

 * <span data-ttu-id="cd1b8-459">如果在生成文件后修改文件，请检查生成工具链和部署机制。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-459">Examine the build toolchain and deployment mechanism in case they're modifying files after the files are built.</span></span> <span data-ttu-id="cd1b8-460">例如，在 Git 转换文件行尾时，如前所述。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-460">An example of this is when Git transforms file line endings, as described earlier.</span></span>
 * <span data-ttu-id="cd1b8-461">如设置为动态修改响应（例如，尝试缩小 HTML），请检查 Web 服务器或 CDN 配置。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-461">Examine the web server or CDN configuration in case they're set up to modify responses dynamically (for example, trying to minify HTML).</span></span> <span data-ttu-id="cd1b8-462">Web 服务器可以实现 HTTP 压缩（例如，返回 `content-encoding: br` 或 `content-encoding: gzip`），因为这不会影响解压缩后的结果。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-462">It's fine for the web server to implement HTTP compression (for example, returning `content-encoding: br` or `content-encoding: gzip`), since this doesn't affect the result after decompression.</span></span> <span data-ttu-id="cd1b8-463">但是，Web 服务器不可以修改未压缩的数据。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-463">However, it's *not* fine for the web server to modify the uncompressed data.</span></span>

### <a name="troubleshoot-integrity-powershell-script"></a><span data-ttu-id="cd1b8-464">完整性 PowerShell 脚本故障排除</span><span class="sxs-lookup"><span data-stu-id="cd1b8-464">Troubleshoot integrity PowerShell script</span></span>

<span data-ttu-id="cd1b8-465">使用 [`integrity.ps1`](https://github.com/dotnet/AspNetCore.Docs/blob/master/aspnetcore/blazor/host-and-deploy/webassembly/_samples/integrity.ps1?raw=true) PowerShell 脚本来验证已发布和已部署的 Blazor 应用。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-465">Use the [`integrity.ps1`](https://github.com/dotnet/AspNetCore.Docs/blob/master/aspnetcore/blazor/host-and-deploy/webassembly/_samples/integrity.ps1?raw=true) PowerShell script to validate a published and deployed Blazor app.</span></span> <span data-ttu-id="cd1b8-466">当应用出现 Blazor 框架无法识别的完整性问题时，该脚本将作为起点提供。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-466">The script is provided as a starting point when the app has integrity issues that the Blazor framework can't identify.</span></span> <span data-ttu-id="cd1b8-467">您的应用可能需要自定义脚本。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-467">Customization of the script might be required for your apps.</span></span>

<span data-ttu-id="cd1b8-468">此脚本将检查 `publish` 文件夹中的文件，并从部署的应用中下载这些文件，以检测包含完整性哈希的不同清单中的问题。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-468">The script checks the files in the `publish` folder and downloaded from the deployed app to detect issues in the different manifests that contain integrity hashes.</span></span> <span data-ttu-id="cd1b8-469">这些检查应检测最常见的问题：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-469">These checks should detect the most common problems:</span></span>

* <span data-ttu-id="cd1b8-470">你修改了已发布的输出中的文件，但未实现它。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-470">You modified a file in the published output without realizing it.</span></span>
* <span data-ttu-id="cd1b8-471">应用未正确部署到部署目标，或者在部署目标的环境中发生了更改。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-471">The app wasn't correctly deployed to the deployment target, or something changed within the deployment target's environment.</span></span>
* <span data-ttu-id="cd1b8-472">部署的应用与发布应用的输出之间存在差异。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-472">There are differences between the deployed app and the output from publishing the app.</span></span>

<span data-ttu-id="cd1b8-473">在 PowerShell 命令行中使用以下命令调用脚本：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-473">Invoke the script with the following command in a PowerShell command shell:</span></span>

```powershell
.\integrity.ps1 {BASE URL} {PUBLISH OUTPUT FOLDER}
```

<span data-ttu-id="cd1b8-474">占位符：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-474">Placeholders:</span></span>

* <span data-ttu-id="cd1b8-475">`{BASE URL}`：已部署的应用的 URL。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-475">`{BASE URL}`: The URL of the deployed app.</span></span>
* <span data-ttu-id="cd1b8-476">`{PUBLISH OUTPUT FOLDER}`：应用的 `publish` 文件夹的路径，或已发布的用于部署应用的位置。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-476">`{PUBLISH OUTPUT FOLDER}`: The path to the app's `publish` folder or location where the app is published for deployment.</span></span>

> [!NOTE]
> <span data-ttu-id="cd1b8-477">若要将 `dotnet/AspNetCore.Docs` GitHub 存储库克隆到使用 [Bitdefender](https://www.bitdefender.com) 病毒扫描程序的系统，请向 `integrity.ps1` 脚本的 Bitdefender 添加例外。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-477">To clone the `dotnet/AspNetCore.Docs` GitHub repository to a system that uses the [Bitdefender](https://www.bitdefender.com) virus scanner, add an exception to Bitdefender for the `integrity.ps1` script.</span></span> <span data-ttu-id="cd1b8-478">在克隆存储库之前，请将例外添加到 Bitdefender，以避免病毒扫描程序隔离该脚本。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-478">Add the exception to Bitdefender before cloning the repo to avoid having the script quarantined by the virus scanner.</span></span> <span data-ttu-id="cd1b8-479">以下示例是 Windows 系统上已克隆的存储库的脚本的典型路径。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-479">The following example is a typical path to the script for the cloned repo on a Windows system.</span></span> <span data-ttu-id="cd1b8-480">请根据需要调整路径。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-480">Adjust the path as needed.</span></span> <span data-ttu-id="cd1b8-481">占位符 `{USER}` 是用户的路径段。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-481">The placeholder `{USER}` is the user's path segment.</span></span>
>
> ```
> C:\Users\{USER}\Documents\GitHub\AspNetCore.Docs\aspnetcore\blazor\host-and-deploy\webassembly\_samples\integrity.ps1
> ```

### <a name="disable-integrity-checking-for-non-pwa-apps"></a><span data-ttu-id="cd1b8-482">禁用非 PWA 应用的完整性检查</span><span class="sxs-lookup"><span data-stu-id="cd1b8-482">Disable integrity checking for non-PWA apps</span></span>

<span data-ttu-id="cd1b8-483">在大多数情况下，不要禁用完整性检查。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-483">In most cases, don't disable integrity checking.</span></span> <span data-ttu-id="cd1b8-484">禁用完整性检查并不能解决导致意外响应的根本问题，并且会导致丢失前面列出的权益。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-484">Disabling integrity checking doesn't solve the underlying problem that has caused the unexpected responses and results in losing the benefits listed earlier.</span></span>

<span data-ttu-id="cd1b8-485">在某些情况下，Web 服务器无法用于返回一致的响应，但别无选择，只能禁用完整性检查。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-485">There may be cases where the web server can't be relied upon to return consistent responses, and you have no choice but to disable integrity checks.</span></span> <span data-ttu-id="cd1b8-486">若要禁用完整性检查，请将以下内容添加到 Blazor WebAssembly 项目的 `.csproj` 文件中的属性组：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-486">To disable integrity checks, add the following to a property group in the Blazor WebAssembly project's `.csproj` file:</span></span>

```xml
<BlazorCacheBootResources>false</BlazorCacheBootResources>
```

<span data-ttu-id="cd1b8-487">`BlazorCacheBootResources` 还会根据 SHA-256 哈希禁用 Blazor 缓存 `.dll`、`.wasm` 和其他文件的默认行为，因为属性指示无法依靠 SHA-256 哈希来确保正确性。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-487">`BlazorCacheBootResources` also disables Blazor's default behavior of caching the `.dll`, `.wasm`, and other files based on their SHA-256 hashes because the property indicates that the SHA-256 hashes can't be relied upon for correctness.</span></span> <span data-ttu-id="cd1b8-488">即使有此设置，浏览器的普通 HTTP 缓存仍可能会缓存这些文件，但是否发生这种情况取决于你的 Web 服务器配置和它所提供的 `cache-control` 标头。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-488">Even with this setting, the browser's normal HTTP cache may still cache those files, but whether or not this happens depends on your web server configuration and the `cache-control` headers that it serves.</span></span>

> [!NOTE]
> <span data-ttu-id="cd1b8-489">`BlazorCacheBootResources` 属性不会禁用[渐进式 Web 应用程序 (PWA)](xref:blazor/progressive-web-app) 的完整性检查。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-489">The `BlazorCacheBootResources` property doesn't disable integrity checks for [Progressive Web Applications (PWAs)](xref:blazor/progressive-web-app).</span></span> <span data-ttu-id="cd1b8-490">有关 PWA 的相关指南，请参阅[禁用 PWA 的完整性检查](#disable-integrity-checking-for-pwas)部分。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-490">For guidance pertaining to PWAs, see the [Disable integrity checking for PWAs](#disable-integrity-checking-for-pwas) section.</span></span>

### <a name="disable-integrity-checking-for-pwas"></a><span data-ttu-id="cd1b8-491">禁用 PWA 的完整性检查</span><span class="sxs-lookup"><span data-stu-id="cd1b8-491">Disable integrity checking for PWAs</span></span>

<span data-ttu-id="cd1b8-492">Blazor 的渐进式 Web 应用程序 (PWA) 模板包含建议的 `service-worker.published.js` 文件，该文件负责获取和存储应用程序文件以供脱机使用。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-492">Blazor's Progressive Web Application (PWA) template contains a suggested `service-worker.published.js` file that's responsible for fetching and storing application files for offline use.</span></span> <span data-ttu-id="cd1b8-493">这是普通应用启动机制的独立进程，具有其自己单独的完整性检查逻辑。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-493">This is a separate process from the normal app startup mechanism and has its own separate integrity checking logic.</span></span>

<span data-ttu-id="cd1b8-494">在 `service-worker.published.js` 文件中，出现以下行：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-494">Inside the `service-worker.published.js` file, following line is present:</span></span>

```javascript
.map(asset => new Request(asset.url, { integrity: asset.hash }));
```

<span data-ttu-id="cd1b8-495">若要禁用完整性检查，请通过将该行更改为以下行来删除 `integrity` 参数：</span><span class="sxs-lookup"><span data-stu-id="cd1b8-495">To disable integrity checking, remove the `integrity` parameter by changing the line to the following:</span></span>

```javascript
.map(asset => new Request(asset.url));
```

<span data-ttu-id="cd1b8-496">同样，禁用完整性检查意味着会丢失完整性检查提供的安全保证。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-496">Again, disabling integrity checking means that you lose the safety guarantees offered by integrity checking.</span></span> <span data-ttu-id="cd1b8-497">例如，如果用户的浏览器在你部署新版本的那一刻缓存应用，则会存在风险，它可能会缓存旧部署中的某些文件和新部署中的某些文件。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-497">For example, there is a risk that if the user's browser is caching the app at the exact moment that you deploy a new version, it could cache some files from the old deployment and some from the new deployment.</span></span> <span data-ttu-id="cd1b8-498">如果发生这种情况，则在部署进一步更新之前，应用程序会在中断状态下停滞。</span><span class="sxs-lookup"><span data-stu-id="cd1b8-498">If that happens, the app becomes stuck in a broken state until you deploy a further update.</span></span>
