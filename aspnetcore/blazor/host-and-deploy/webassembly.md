...
<span data-ttu-id="17a6f-108">支持以下部署策略：</span><span class="sxs-lookup"><span data-stu-id="17a6f-108">The following deployment strategies are supported:</span></span>
...
<span data-ttu-id="17a6f-147">由于浏览器针对客户端页面请求基于 Internet 的主机，因此 Web 服务器和托管服务必须将对服务器上非物理方式资源的所有请求重写为 `index.html` 页。</span><span class="sxs-lookup"><span data-stu-id="17a6f-147">Because browsers make requests to Internet-based hosts for client-side pages, web servers and hosting services must rewrite all requests for resources not physically on the server to the `index.html` page.</span></span> <span data-ttu-id="17a6f-148">如果返回 `index.html`，应用的 Blazor 路由器将接管工作并使用正确的资源响应。</span><span class="sxs-lookup"><span data-stu-id="17a6f-148">When `index.html` is returned, the app's Blazor router takes over and responds with the correct resource.</span></span>
...
### <a name="app-configuration"></a><span data-ttu-id="17a6f-160">应用配置</span><span class="sxs-lookup"><span data-stu-id="17a6f-160">App configuration</span></span>
...
* <span data-ttu-id="17a6f-164">向解决方案添加第二个客户端应用：</span><span class="sxs-lookup"><span data-stu-id="17a6f-164">Add a second client app to the solution:</span></span>
  ...
  * <span data-ttu-id="17a6f-167">在应用的项目文件中，执行以下操作：</span><span class="sxs-lookup"><span data-stu-id="17a6f-167">In the app's project file:</span></span>
    ...
    * <span data-ttu-id="17a6f-169">向 `Shared` 项目添加一个项目引用：</span><span class="sxs-lookup"><span data-stu-id="17a6f-169">Add a project reference to the `Shared` project:</span></span>
      ...
      <span data-ttu-id="17a6f-170">占位符 `{SOLUTION NAME}` 是解决方案的名称。</span><span class="sxs-lookup"><span data-stu-id="17a6f-170">The placeholder `{SOLUTION NAME}` is the solution's name.</span></span>
...
<span data-ttu-id="17a6f-202">客户端应用的 `wwwroot/index.html` 文件要求库的样式表添加以下 `<link>` 标记：</span><span class="sxs-lookup"><span data-stu-id="17a6f-202">The client app's `wwwroot/index.html` file requests the library's stylesheet with the following added `<link>` tag:</span></span>
...
### <a name="iis"></a><span data-ttu-id="17a6f-214">IIS</span><span class="sxs-lookup"><span data-stu-id="17a6f-214">IIS</span></span>
...
* <span data-ttu-id="17a6f-249">`web.config` 文件，IIS 使用该文件配置网站，包括所需的重定向规则和文件内容类型。</span><span class="sxs-lookup"><span data-stu-id="17a6f-249">The `web.config` file that IIS uses to configure the website, including the required redirect rules and file content types.</span></span>
...
#### <a name="brotli-and-gzip-compression"></a><span data-ttu-id="17a6f-258">Brotli 和 Gzip 压缩</span><span class="sxs-lookup"><span data-stu-id="17a6f-258">Brotli and Gzip compression</span></span>
...
### <a name="azure-storage"></a><span data-ttu-id="17a6f-273">Azure 存储</span><span class="sxs-lookup"><span data-stu-id="17a6f-273">Azure Storage</span></span>
...
### <a name="nginx-in-docker"></a><span data-ttu-id="17a6f-295">Docker 中的 Nginx</span><span class="sxs-lookup"><span data-stu-id="17a6f-295">Nginx in Docker</span></span>
...
1. <span data-ttu-id="17a6f-303">将 Apache 配置文件放入 `/etc/httpd/conf.d/` 目录（这是 CentOS 7 中的默认 Apache 配置目录）。</span><span class="sxs-lookup"><span data-stu-id="17a6f-303">Place the Apache configuration file into the `/etc/httpd/conf.d/` directory, which is the default Apache configuration directory in CentOS 7.</span></span>
...
## <a name="host-configuration-values"></a><span data-ttu-id="17a6f-315">主机配置值</span><span class="sxs-lookup"><span data-stu-id="17a6f-315">Host configuration values</span></span>
...
* <span data-ttu-id="17a6f-340">以本地方式在命令提示符下运行应用时传递该参数。</span><span class="sxs-lookup"><span data-stu-id="17a6f-340">Pass the argument when running the app locally at a command prompt.</span></span> <span data-ttu-id="17a6f-341">在应用的目录中，执行以下操作：</span><span class="sxs-lookup"><span data-stu-id="17a6f-341">From the app's directory, execute:</span></span>
...
<span data-ttu-id="17a6f-380">发布应用后，使用 shell 脚本或 DevOps 生成管道将 `.dll` 文件重命名，以使用其他文件扩展名。</span><span class="sxs-lookup"><span data-stu-id="17a6f-380">After publishing the app, use a shell script or DevOps build pipeline to rename `.dll` files to use a different file extension.</span></span> <span data-ttu-id="17a6f-381">将 `.dll` 文件的目标位置设为应用的已发布输出的 `wwwroot` 目录中（例如 `{CONTENT ROOT}/bin/Release/netstandard2.1/publish/wwwroot`）。</span><span class="sxs-lookup"><span data-stu-id="17a6f-381">Target the `.dll` files in the `wwwroot` directory of the app's published output (for example, `{CONTENT ROOT}/bin/Release/netstandard2.1/publish/wwwroot`).</span></span>

<span data-ttu-id="17a6f-382">在下面的示例中，重命名 `.dll` 文件，以使用 `.bin` 文件扩展名。</span><span class="sxs-lookup"><span data-stu-id="17a6f-382">In the following examples, `.dll` files are renamed to use the `.bin` file extension.</span></span>

<span data-ttu-id="17a6f-383">在 Windows 上：</span><span class="sxs-lookup"><span data-stu-id="17a6f-383">On Windows:</span></span>
...
<span data-ttu-id="17a6f-384">如果服务工作进程资产也在使用中，请添加以下命令：</span><span class="sxs-lookup"><span data-stu-id="17a6f-384">If service worker assets are also in use, add the following command:</span></span>

```powershell
((Get-Content .\service-worker-assets.js -Raw) -replace '.dll"','.bin"') | Set-Content .\service-worker-assets.js
```

<span data-ttu-id="17a6f-385">在 Linux 或 macOS 上：</span><span class="sxs-lookup"><span data-stu-id="17a6f-385">On Linux or macOS:</span></span>
...
<span data-ttu-id="17a6f-386">如果服务工作进程资产也在使用中，请添加以下命令：</span><span class="sxs-lookup"><span data-stu-id="17a6f-386">If service worker assets are also in use, add the following command:</span></span>

```console
sed -i 's/\.dll"/.bin"/g' service-worker-assets.js
```
   
<span data-ttu-id="17a6f-387">要使用不同于 `.bin` 的其他文件扩展名，请在前面的命令中替换 `.bin`。</span><span class="sxs-lookup"><span data-stu-id="17a6f-387">To use a different file extension than `.bin`, replace `.bin` in the preceding commands.</span></span>

<span data-ttu-id="17a6f-388">若要处理压缩的 `blazor.boot.json.gz` 和 `blazor.boot.json.br` 文件，请采用以下方法之一：</span><span class="sxs-lookup"><span data-stu-id="17a6f-388">To address the compressed `blazor.boot.json.gz` and `blazor.boot.json.br` files, adopt either of the following approaches:</span></span>

* <span data-ttu-id="17a6f-389">删除压缩的 `blazor.boot.json.gz` 和 `blazor.boot.json.br` 文件。</span><span class="sxs-lookup"><span data-stu-id="17a6f-389">Remove the compressed `blazor.boot.json.gz` and `blazor.boot.json.br` files.</span></span> <span data-ttu-id="17a6f-390">此方法禁用压缩。</span><span class="sxs-lookup"><span data-stu-id="17a6f-390">Compression is disabled with this approach.</span></span>
* <span data-ttu-id="17a6f-391">重新压缩更新后的 `blazor.boot.json` 文件。</span><span class="sxs-lookup"><span data-stu-id="17a6f-391">Recompress the updated `blazor.boot.json` file.</span></span>

<span data-ttu-id="17a6f-392">上述指导也适用于正在使用服务工作进程资产的情况。</span><span class="sxs-lookup"><span data-stu-id="17a6f-392">The preceding guidance also applies when service worker assets are in use.</span></span> <span data-ttu-id="17a6f-393">删除或重新压缩 `wwwroot/service-worker-assets.js.br` 和 `wwwroot/service-worker-assets.js.gz`。</span><span class="sxs-lookup"><span data-stu-id="17a6f-393">Remove or recompress `wwwroot/service-worker-assets.js.br` and `wwwroot/service-worker-assets.js.gz`.</span></span> <span data-ttu-id="17a6f-394">否则，浏览器中的文件完整性检查将失败。</span><span class="sxs-lookup"><span data-stu-id="17a6f-394">Otherwise, file integrity checks fail in the browser.</span></span>

<span data-ttu-id="17a6f-395">以下 Windows 示例使用项目根目录中的 PowerShell 脚本。</span><span class="sxs-lookup"><span data-stu-id="17a6f-395">The following Windows example uses a PowerShell script placed at the root of the project.</span></span>

<span data-ttu-id="17a6f-396">`ChangeDLLExtensions.ps1:`:</span><span class="sxs-lookup"><span data-stu-id="17a6f-396">`ChangeDLLExtensions.ps1:`:</span></span>
...
<span data-ttu-id="17a6f-397">如果服务工作进程资产也在使用中，请添加以下命令：</span><span class="sxs-lookup"><span data-stu-id="17a6f-397">If service worker assets are also in use, add the following command:</span></span>

```powershell
((Get-Content $filepath\bin\Release\$tfm\wwwroot\service-worker-assets.js -Raw) -replace '.dll"','.bin"') | Set-Content $filepath\bin\Release\$tfm\wwwroot\service-worker-assets.js
```

<span data-ttu-id="17a6f-398">在项目文件中，在发布应用后运行脚本：</span><span class="sxs-lookup"><span data-stu-id="17a6f-398">In the project file, the script is run after publishing the app:</span></span>
...
> [!NOTE]
> <span data-ttu-id="17a6f-399">重命名和延迟加载相同的程序集时，请参阅 <xref:blazor/webassembly-lazy-load-assemblies#onnavigateasync-events-and-renamed-assembly-files> 中的指南。</span><span class="sxs-lookup"><span data-stu-id="17a6f-399">When renaming and lazy loading the same assemblies, see the guidance in <xref:blazor/webassembly-lazy-load-assemblies#onnavigateasync-events-and-renamed-assembly-files>.</span></span>

## <a name="resolve-integrity-check-failures"></a><span data-ttu-id="17a6f-400">解决完整性检查失败</span><span class="sxs-lookup"><span data-stu-id="17a6f-400">Resolve integrity check failures</span></span>

<span data-ttu-id="17a6f-401">当 Blazor WebAssembly 下载应用的启动文件时，它会指示浏览器对响应执行完整性检查。</span><span class="sxs-lookup"><span data-stu-id="17a6f-401">When Blazor WebAssembly downloads an app's startup files, it instructs the browser to perform integrity checks on the responses.</span></span> <span data-ttu-id="17a6f-402">它使用 `blazor.boot.json` 文件中的信息为 `.dll`、`.wasm` 和其他文件指定预期的 SHA-256 哈希值。</span><span class="sxs-lookup"><span data-stu-id="17a6f-402">It uses information in the `blazor.boot.json` file to specify the expected SHA-256 hash values for `.dll`, `.wasm`, and other files.</span></span> <span data-ttu-id="17a6f-403">这对以下原因有所帮助：</span><span class="sxs-lookup"><span data-stu-id="17a6f-403">This is beneficial for the following reasons:</span></span>

* <span data-ttu-id="17a6f-404">它可确保不会出现加载不一致文件集的风险，例如，在用户正在下载应用程序文件时将新部署应用到 Web 服务器的情况。</span><span class="sxs-lookup"><span data-stu-id="17a6f-404">It ensures you don't risk loading an inconsistent set of files, for example if a new deployment is applied to your web server while the user is in the process of downloading the application files.</span></span> <span data-ttu-id="17a6f-405">不一致的文件可能导致未定义的行为。</span><span class="sxs-lookup"><span data-stu-id="17a6f-405">Inconsistent files could lead to undefined behavior.</span></span>
* <span data-ttu-id="17a6f-406">它可确保用户的浏览器从不缓存不一致或无效的响应，这些响应可能会阻止他们启动应用（即使他们手动刷新了页面也是如此）。</span><span class="sxs-lookup"><span data-stu-id="17a6f-406">It ensures the user's browser never caches inconsistent or invalid responses, which could prevent them from starting the app even if they manually refresh the page.</span></span>
* <span data-ttu-id="17a6f-407">它可以安全地缓存响应，甚至无需检查服务器端更改，直到预期的 SHA-256 哈希本身发生更改，因此，后续页面加载需要较少的请求即可快速完成。</span><span class="sxs-lookup"><span data-stu-id="17a6f-407">It makes it safe to cache the responses and not even check for server-side changes until the expected SHA-256 hashes themselves change, so subsequent page loads involve fewer requests and complete much faster.</span></span>

<span data-ttu-id="17a6f-408">如果你的 Web 服务器返回的响应与预期的 SHA-256 哈希不匹配，你将看到类似于以下内容的错误显示在浏览器的开发人员控制台中：</span><span class="sxs-lookup"><span data-stu-id="17a6f-408">If your web server returns responses that don't match the expected SHA-256 hashes, you will see an error similar to the following appear in the browser's developer console:</span></span>
...
<span data-ttu-id="17a6f-409">在大多数情况下，这不是完整性检查本身的问题。</span><span class="sxs-lookup"><span data-stu-id="17a6f-409">In most cases, this is *not* a problem with integrity checking itself.</span></span> <span data-ttu-id="17a6f-410">相反，它表示存在其他问题，并且完整性检查会警告你其他问题。</span><span class="sxs-lookup"><span data-stu-id="17a6f-410">Instead, it means there is some other problem, and the integrity check is warning you about that other problem.</span></span>

### <a name="diagnosing-integrity-problems"></a><span data-ttu-id="17a6f-411">诊断完整性问题</span><span class="sxs-lookup"><span data-stu-id="17a6f-411">Diagnosing integrity problems</span></span>

<span data-ttu-id="17a6f-412">生成应用时，生成的 `blazor.boot.json` 清单将描述生成输出生成时启动资源（例如，`.dll`、`.wasm` 和其他文件）的 SHA-256 哈希。</span><span class="sxs-lookup"><span data-stu-id="17a6f-412">When an app is built, the generated `blazor.boot.json` manifest describes the SHA-256 hashes of your boot resources (for example, `.dll`, `.wasm`, and other files) at the time that the build output is produced.</span></span> <span data-ttu-id="17a6f-413">只要 `blazor.boot.json` 中的 SHA-256 哈希与传递到浏览器的文件相匹配，完整性检查就会通过。</span><span class="sxs-lookup"><span data-stu-id="17a6f-413">The integrity check passes as long as the SHA-256 hashes in `blazor.boot.json` match the files delivered to the browser.</span></span>

<span data-ttu-id="17a6f-414">此失败的常见原因包括：</span><span class="sxs-lookup"><span data-stu-id="17a6f-414">Common reasons why this fails are:</span></span>

 * <span data-ttu-id="17a6f-415">Web 服务器的响应是一个错误（例如，“404 - 找不到”或“500 - 内部服务器错误”），而不是浏览器所请求的文件。</span><span class="sxs-lookup"><span data-stu-id="17a6f-415">The web server's response is an error (for example, a *404 - Not Found* or a *500 - Internal Server Error*) instead of the file the browser requested.</span></span> <span data-ttu-id="17a6f-416">浏览器会将其报告为完整性检查失败，而不是响应失败。</span><span class="sxs-lookup"><span data-stu-id="17a6f-416">This is reported by the browser as an integrity check failure and not as a response failure.</span></span>
 * <span data-ttu-id="17a6f-417">在文件生成和传递到浏览器之间已更改文件的内容。</span><span class="sxs-lookup"><span data-stu-id="17a6f-417">Something has changed the contents of the files between the build and delivery of the files to the browser.</span></span> <span data-ttu-id="17a6f-418">下面可能会发生这种情况：</span><span class="sxs-lookup"><span data-stu-id="17a6f-418">This might happen:</span></span>
   * <span data-ttu-id="17a6f-419">你或生成工具手动修改生成输出的情况。</span><span class="sxs-lookup"><span data-stu-id="17a6f-419">If you or build tools manually modify the build output.</span></span>
   * <span data-ttu-id="17a6f-420">部署过程的某个方面修改了文件的情况。</span><span class="sxs-lookup"><span data-stu-id="17a6f-420">If some aspect of the deployment process modified the files.</span></span> <span data-ttu-id="17a6f-421">例如，在使用基于 Git 的部署机制时，请记住，如果你在 Windows 上提交文件并在 Linux 上检查它们，则 Git 会以透明方式将 Windows 样式的行尾转换为 Unix 样式的行尾。</span><span class="sxs-lookup"><span data-stu-id="17a6f-421">For example if you use a Git-based deployment mechanism, bear in mind that Git transparently converts Windows-style line endings to Unix-style line endings if you commit files on Windows and check them out on Linux.</span></span> <span data-ttu-id="17a6f-422">更改文件行尾将更改 SHA-256 哈希。</span><span class="sxs-lookup"><span data-stu-id="17a6f-422">Changing file line endings change the SHA-256 hashes.</span></span> <span data-ttu-id="17a6f-423">若要避免此问题，请考虑[使用 `.gitattributes` 将生成项目视为 `binary` 文件](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes)。</span><span class="sxs-lookup"><span data-stu-id="17a6f-423">To avoid this problem, consider [using `.gitattributes` to treat build artifacts as `binary` files](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes).</span></span>
   * <span data-ttu-id="17a6f-424">Web 服务器在提供文件内容的过程中对其进行修改。</span><span class="sxs-lookup"><span data-stu-id="17a6f-424">The web server modifies the file contents as part of serving them.</span></span> <span data-ttu-id="17a6f-425">例如，某些内容分发网络 (CDN) 会自动尝试[缩小](xref:client-side/bundling-and-minification#minification) HTML，从而对其进行修改。</span><span class="sxs-lookup"><span data-stu-id="17a6f-425">For example, some content distribution networks (CDNs) automatically attempt to [minify](xref:client-side/bundling-and-minification#minification) HTML, thereby modifying it.</span></span> <span data-ttu-id="17a6f-426">可能需要禁用此类功能。</span><span class="sxs-lookup"><span data-stu-id="17a6f-426">You may need to disable such features.</span></span>

<span data-ttu-id="17a6f-427">若要诊断哪些功能适用于你的情况，请执行执行操作：</span><span class="sxs-lookup"><span data-stu-id="17a6f-427">To diagnose which of these applies in your case:</span></span>

 1. <span data-ttu-id="17a6f-428">通过读取错误消息来记下哪个文件触发错误。</span><span class="sxs-lookup"><span data-stu-id="17a6f-428">Note which file is triggering the error by reading the error message.</span></span>
 1. <span data-ttu-id="17a6f-429">打开浏览器的开发人员工具，然后在“网络”选项卡中查找。如有必要，请重新加载页面以查看请求和响应的列表。</span><span class="sxs-lookup"><span data-stu-id="17a6f-429">Open your browser's developer tools and look in the *Network* tab. If necessary, reload the page to see the list of requests and responses.</span></span> <span data-ttu-id="17a6f-430">在该列表中查找触发错误的文件。</span><span class="sxs-lookup"><span data-stu-id="17a6f-430">Find the file that is triggering the error in that list.</span></span>
 1. <span data-ttu-id="17a6f-431">检查响应中的 HTTP 状态代码。</span><span class="sxs-lookup"><span data-stu-id="17a6f-431">Check the HTTP status code in the response.</span></span> <span data-ttu-id="17a6f-432">如果服务器返回除“200 - 正常”（或其他 2xx 状态代码）以外的任何内容，则需要诊断服务器端问题。</span><span class="sxs-lookup"><span data-stu-id="17a6f-432">If the server returns anything other than *200 - OK* (or another 2xx status code), then you have a server-side problem to diagnose.</span></span> <span data-ttu-id="17a6f-433">例如，状态代码 403 表示存在授权问题，而状态代码 500 表示服务器以未指定的方式失败。</span><span class="sxs-lookup"><span data-stu-id="17a6f-433">For example, status code 403 means there's an authorization problem, whereas status code 500 means the server is failing in an unspecified manner.</span></span> <span data-ttu-id="17a6f-434">请参阅服务器端日志以诊断和修复应用。</span><span class="sxs-lookup"><span data-stu-id="17a6f-434">Consult server-side logs to diagnose and fix the app.</span></span>
 1. <span data-ttu-id="17a6f-435">如果资源的状态代码为“200 - 正常”，请在浏览器的开发人员工具中查看响应内容，并检查内容是否与预期的数据匹配。</span><span class="sxs-lookup"><span data-stu-id="17a6f-435">If the status code is *200 - OK* for the resource, look at the response content in browser's developer tools and check that the content matchs up with the data expected.</span></span> <span data-ttu-id="17a6f-436">例如，常见问题是错误配置了路由，因此请求甚至返回其他文件的 `index.html` 数据。</span><span class="sxs-lookup"><span data-stu-id="17a6f-436">For example, a common problem is to misconfigure routing so that requests return your `index.html` data even for other files.</span></span> <span data-ttu-id="17a6f-437">请确保对 `.wasm` 请求的响应是 WebAssembly 二进制文件，对 `.dll` 请求的响应是 .NET 程序集二进制文件。</span><span class="sxs-lookup"><span data-stu-id="17a6f-437">Make sure that responses to `.wasm` requests are WebAssembly binaries and that responses to `.dll` requests are .NET assembly binaries.</span></span> <span data-ttu-id="17a6f-438">如果不是，则需要诊断服务器端路由问题。</span><span class="sxs-lookup"><span data-stu-id="17a6f-438">If not, you have a server-side routing problem to diagnose.</span></span>

<span data-ttu-id="17a6f-439">如果确认服务器返回看似正确的数据，则必须在生成文件和传递文件之间修改内容。</span><span class="sxs-lookup"><span data-stu-id="17a6f-439">If you confirm that the server is returning plausibly correct data, there must be something else modifying the contents in between build and delivery of the file.</span></span> <span data-ttu-id="17a6f-440">若要对此进行调查，请执行以下操作：</span><span class="sxs-lookup"><span data-stu-id="17a6f-440">To investigate this:</span></span>

 * <span data-ttu-id="17a6f-441">如果在生成文件后修改文件，请检查生成工具链和部署机制。</span><span class="sxs-lookup"><span data-stu-id="17a6f-441">Examine the build toolchain and deployment mechanism in case they're modifying files after the files are built.</span></span> <span data-ttu-id="17a6f-442">例如，在 Git 转换文件行尾时，如前所述。</span><span class="sxs-lookup"><span data-stu-id="17a6f-442">An example of this is when Git transforms file line endings, as described earlier.</span></span>
 * <span data-ttu-id="17a6f-443">如设置为动态修改响应（例如，尝试缩小 HTML），请检查 Web 服务器或 CDN 配置。</span><span class="sxs-lookup"><span data-stu-id="17a6f-443">Examine the web server or CDN configuration in case they're set up to modify responses dynamically (for example, trying to minify HTML).</span></span> <span data-ttu-id="17a6f-444">Web 服务器可以实现 HTTP 压缩（例如，返回 `content-encoding: br` 或 `content-encoding: gzip`），因为这不会影响解压缩后的结果。</span><span class="sxs-lookup"><span data-stu-id="17a6f-444">It's fine for the web server to implement HTTP compression (for example, returning `content-encoding: br` or `content-encoding: gzip`), since this doesn't affect the result after decompression.</span></span> <span data-ttu-id="17a6f-445">但是，Web 服务器不可以修改未压缩的数据。</span><span class="sxs-lookup"><span data-stu-id="17a6f-445">However, it's *not* fine for the web server to modify the uncompressed data.</span></span>

### <a name="disable-integrity-checking-for-non-pwa-apps"></a><span data-ttu-id="17a6f-446">禁用非 PWA 应用的完整性检查</span><span class="sxs-lookup"><span data-stu-id="17a6f-446">Disable integrity checking for non-PWA apps</span></span>

<span data-ttu-id="17a6f-447">在大多数情况下，不要禁用完整性检查。</span><span class="sxs-lookup"><span data-stu-id="17a6f-447">In most cases, don't disable integrity checking.</span></span> <span data-ttu-id="17a6f-448">禁用完整性检查并不能解决导致意外响应的根本问题，并且会导致丢失前面列出的权益。</span><span class="sxs-lookup"><span data-stu-id="17a6f-448">Disabling integrity checking doesn't solve the underlying problem that has caused the unexpected responses and results in losing the benefits listed earlier.</span></span>

<span data-ttu-id="17a6f-449">在某些情况下，Web 服务器无法用于返回一致的响应，但别无选择，只能禁用完整性检查。</span><span class="sxs-lookup"><span data-stu-id="17a6f-449">There may be cases where the web server can't be relied upon to return consistent responses, and you have no choice but to disable integrity checks.</span></span> <span data-ttu-id="17a6f-450">若要禁用完整性检查，请将以下内容添加到 Blazor WebAssembly 项目的 `.csproj` 文件中的属性组：</span><span class="sxs-lookup"><span data-stu-id="17a6f-450">To disable integrity checks, add the following to a property group in the Blazor WebAssembly project's `.csproj` file:</span></span>

```xml
<BlazorCacheBootResources>false</BlazorCacheBootResources>
```

<span data-ttu-id="17a6f-451">`BlazorCacheBootResources` 还会根据 SHA-256 哈希禁用 Blazor 缓存 `.dll`、`.wasm` 和其他文件的默认行为，因为属性指示无法依靠 SHA-256 哈希来确保正确性。</span><span class="sxs-lookup"><span data-stu-id="17a6f-451">`BlazorCacheBootResources` also disables Blazor's default behavior of caching the `.dll`, `.wasm`, and other files based on their SHA-256 hashes because the property indicates that the SHA-256 hashes can't be relied upon for correctness.</span></span> <span data-ttu-id="17a6f-452">即使有此设置，浏览器的普通 HTTP 缓存仍可能会缓存这些文件，但是否发生这种情况取决于你的 Web 服务器配置和它所提供的 `cache-control` 标头。</span><span class="sxs-lookup"><span data-stu-id="17a6f-452">Even with this setting, the browser's normal HTTP cache may still cache those files, but whether or not this happens depends on your web server configuration and the `cache-control` headers that it serves.</span></span>

> [!NOTE]
> <span data-ttu-id="17a6f-453">`BlazorCacheBootResources` 属性不会禁用[渐进式 Web 应用程序 (PWA)](xref:blazor/progressive-web-app) 的完整性检查。</span><span class="sxs-lookup"><span data-stu-id="17a6f-453">The `BlazorCacheBootResources` property doesn't disable integrity checks for [Progressive Web Applications (PWAs)](xref:blazor/progressive-web-app).</span></span> <span data-ttu-id="17a6f-454">有关 PWA 的相关指南，请参阅[禁用 PWA 的完整性检查](#disable-integrity-checking-for-pwas)部分。</span><span class="sxs-lookup"><span data-stu-id="17a6f-454">For guidance pertaining to PWAs, see the [Disable integrity checking for PWAs](#disable-integrity-checking-for-pwas) section.</span></span>

### <a name="disable-integrity-checking-for-pwas"></a><span data-ttu-id="17a6f-455">禁用 PWA 的完整性检查</span><span class="sxs-lookup"><span data-stu-id="17a6f-455">Disable integrity checking for PWAs</span></span>

<span data-ttu-id="17a6f-456">Blazor 的渐进式 Web 应用程序 (PWA) 模板包含建议的 `service-worker.published.js` 文件，该文件负责获取和存储应用程序文件以供脱机使用。</span><span class="sxs-lookup"><span data-stu-id="17a6f-456">Blazor's Progressive Web Application (PWA) template contains a suggested `service-worker.published.js` file that's responsible for fetching and storing application files for offline use.</span></span> <span data-ttu-id="17a6f-457">这是普通应用启动机制的独立进程，具有其自己单独的完整性检查逻辑。</span><span class="sxs-lookup"><span data-stu-id="17a6f-457">This is a separate process from the normal app startup mechanism and has its own separate integrity checking logic.</span></span>

<span data-ttu-id="17a6f-458">在 `service-worker.published.js` 文件中，出现以下行：</span><span class="sxs-lookup"><span data-stu-id="17a6f-458">Inside the `service-worker.published.js` file, following line is present:</span></span>

```javascript
.map(asset => new Request(asset.url, { integrity: asset.hash }));
```

<span data-ttu-id="17a6f-459">若要禁用完整性检查，请通过将该行更改为以下行来删除 `integrity` 参数：</span><span class="sxs-lookup"><span data-stu-id="17a6f-459">To disable integrity checking, remove the `integrity` parameter by changing the line to the following:</span></span>

```javascript
.map(asset => new Request(asset.url));
```

<span data-ttu-id="17a6f-460">同样，禁用完整性检查意味着会丢失完整性检查提供的安全保证。</span><span class="sxs-lookup"><span data-stu-id="17a6f-460">Again, disabling integrity checking means that you lose the safety guarantees offered by integrity checking.</span></span> <span data-ttu-id="17a6f-461">例如，如果用户的浏览器在你部署新版本的那一刻缓存应用，则会存在风险，它可能会缓存旧部署中的某些文件和新部署中的某些文件。</span><span class="sxs-lookup"><span data-stu-id="17a6f-461">For example, there is a risk that if the user's browser is caching the app at the exact moment that you deploy a new version, it could cache some files from the old deployment and some from the new deployment.</span></span> <span data-ttu-id="17a6f-462">如果发生这种情况，则在部署进一步更新之前，应用程序会在中断状态下停滞。</span><span class="sxs-lookup"><span data-stu-id="17a6f-462">If that happens, the app becomes stuck in a broken state until you deploy a further update.</span></span>
