...
> [!NOTE]
> 不要**直接**在 lambda 表达式中的`i` `for`循环中使用循环变量（）。 否则，所有 lambda 表达式都将使用相同的变量`i`，从而使所有 lambda 中的值都相同。 始终捕获其在本地变量中的值`buttonNumber` （在前面的示例中），然后使用它。
...
Blazor 使用`SynchronizationContext`来强制执行单个逻辑线程。 对此`SynchronizationContext`执行 Blazor 引发的组件生命周期方法和任何事件回调。 如果组件必须根据外部事件（如计时器或其他通知）进行更新，请使用`InvokeAsync`方法，该方法将调度到 Blazor 的。 `SynchronizationContext`
...
在前面的示例中`NotifierService` ，在 Blazor 的`OnNotify` `SynchronizationContext`外部调用该组件的方法。 `InvokeAsync`用于切换到正确的上下文，并将呈现器排队。
...
若要将同一类型的多个值层叠到相同的子树中`Name` ，请为`CascadingValue`每个组件及其`CascadingParameter`相应的提供一个唯一字符串。 在下面的示例中， `CascadingValue`两个组件按名称`MyCascadingType`层叠了不同的实例：
...
由于 Blazor 呈现 HTML，因此支持浏览器支持的图像（包括可缩放的向量图形（svg）图像（*svg*）） `<img>`是通过标记支持的：
...
但是，在所有情况下均不支持内联 SVG 标记。 如果将`<svg>`标记直接放入组件文件（*razor*），则支持基本图像呈现，但尚不支持很多高级方案。 例如， `<use>`标记当前未遵循，并且`@bind`不能与一些 SVG 标记一起使用。 我们预计会在将来的版本中解决这些限制。
