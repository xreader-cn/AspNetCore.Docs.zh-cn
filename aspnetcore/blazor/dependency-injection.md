...
在*Program.cs*的 `Main` 方法中配置应用服务集合的服务。 在下面的示例中，为 `IMyDependency`注册了 `MyDependency` 实现：
...
主机还提供应用的中央配置实例。 基于前面的示例，天气服务的 URL 将从默认配置源（例如*appsettings*）传递到 `InitializeWeatherAsync`：
